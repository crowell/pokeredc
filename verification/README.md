...
- `FarCopyData2`: proven for the BC (0x60) copied bytes (`m_copy` = the data written from `[HL]` to `[DE]`) across all general-register and canonical-flag inputs; HL (source), DE (dest) and BC (length) are fixed concrete so the copy addresses and length are concrete while the BC source bytes stay symbolic and shared between the asm and native paths (the A source-bank byte is unconstrained and only drives the no-op bank switch). It is identical to `FarCopyData` except it stashes the source ROM bank in `hROMBankTemp` rather than `wBuffer`; the bank switch `ldh [a8],a` (0xE0, writing hROMBankTemp/hLoadedROMBank), `ld [a16],a` (0xEA, writing rROMB), and `ldh a,[a8]` (0xF0, reading hROMBankTemp/hLoadedROMBank) are shimmed (no-op for the observable in the flat memory model), `call CopyData` is modeled as a BC-byte forward copy from `[HL]` to `[DE]` (CopyDataSim), and `push af`/`pop af`/`ret` run natively.
- `GetMonFieldMoves`: proven for the four observable bytes (`m_wfieldmoves` = `wFieldMoves[0..3]` name indices of the party mon's field moves, `m_nfm` = `wNumFieldMoves`, `m_leftmost` = `wFieldMovesLeftmostXCoord` = minimum field-move xcoord, `m_lastid` = `wLastFieldMoveID`) across all general-register and canonical-flag inputs; the party mon index `[wWhichPokemon]` is fixed concrete (0), and the four party-move bytes plus the concrete `FieldMoveDisplayData` ROM table make the walk branch-free while the four general registers stay symbolic. `call AddNTimes` is modeled as `hl = hl + bc*a`; `ld a,[a16]` (0xFA, reading `wWhichPokemon`/`wNumFieldMoves`/`wFieldMovesLeftmostXCoord`/`wLastFieldMoveID`), `ld [a16],a` (0xEA, writing `wLastFieldMoveID`/`wNumFieldMoves`/`wFieldMovesLeftmostXCoord`), `ld a,[hli]` (0x2A, reading the table) and `ld [hli],a` (0x22, appending the name index) are shimmed (absent from z80), and `ld hl,nn`/`ld bc,nn`/`ld c,imm`/`ld d,h`/`ld e,l`/`ld a,[de]`/`and a`/`ld b,a`/`inc de`/`cp $ff`/`cp b`/`inc hl`/`pop hl`/`push hl`/`jr`/`ret` run natively.
