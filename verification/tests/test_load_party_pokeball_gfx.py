...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        self.state.regs.d = claripy.BVV(0x69, 8)
        self.state.regs.e = claripy.BVV(0x7E, 8)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "LoadPartyPokeballGfx")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_load_party_pokeball_gfx")
...
def test_load_party_pokeball_gfx_pathwise_equivalence() -> None:
    values = symbolic_registers("load_pokeball_gfx")
