...
class DecB(angr.SimProcedure):
    """Models `dec b` with carry preservation (see CopyMapConnectionHeader)."""
...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "ClearSprites")
    q = symbol_location(SYMBOLS, "ClearSprites.loop").address
    p = angr.Project(
    ...
    p.hook(q, StoreIncHl(q + 1), length=1)  # ld [hli], a
    p.hook(q + 1, DecB(DONE), length=1)      # dec b
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_clear_sprites_step")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "ClearSprites")
    assert linked_bytes(ROM, loc, 11) == bytes.fromhex("af2100c306a0220520fcc9")
