...
def begin(i):
 loc=symbol_location(SYMBOLS,"CopyMapConnectionHeader");p=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":loc.address});p.hook(loc.address+2,Bound(LOOP),length=1);s=p.factory.blank_state(addr=loc.address);set_assembly_registers(s,i);m=p.factory.simulation_manager(s);m.explore(find=LOOP);x=m.found[0];return E(**assembly_registers(x),memory=claripy.Concat(i["fetched"],i["written"]),cont=claripy.BVV(1,8),constraints=tuple(x.solver.constraints))
def step(i):
 loc=symbol_location(SYMBOLS,"CopyMapConnectionHeader");loop=loc.address+2;p=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":loop});p.hook(loop,Fetch(loop+1),length=1);p.hook(loop+1,Store(loop+2),length=1);p.hook(loop+3,Sm83DecRegister("c",loop+4),length=1);p.hook(loop+6,Bound(RETURN),length=1);s=p.factory.blank_state(addr=loop);set_assembly_registers(s,i);s.globals["fetched"]=i["fetched"];s.globals["written"]=i["written"];m=p.factory.simulation_manager(s);m.stashes["found"]=[]
 while m.active:
...
def test_begin():
 i=inputs("connection_header_begin");assert_pathwise_equivalent([begin(i)],native("port_copy_map_connection_header_begin",i),(*REGISTERS,"memory","cont"))
@pytest.mark.skipif(not NATIVE_ELF.exists(),reason="native")
def test_step():
 i=inputs("connection_header_step");assert_pathwise_equivalent(step(i),native("port_copy_map_connection_header_step",i,True),(*REGISTERS,"memory","cont"))
def test_body():
 loc=symbol_location(SYMBOLS,"CopyMapConnectionHeader");assert linked_bytes(ROM,loc,9)==bytes.fromhex("0e0b2a12130d20fac9")
