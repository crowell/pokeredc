...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.l = claripy.BVV(0x7A, 8)
        self.state.regs.d = claripy.BVV(0xFF, 8)
        self.state.regs.e = claripy.BVV(0xFF, 8)
        self.state.regs.b = claripy.BVV(0, 8)
        self.state.regs.c = claripy.BVV(3, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "PlacePlayerHUDTiles")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_place_player_hud_tiles")
...
def test_place_player_hud_tiles_pathwise_equivalence() -> None:
    values = symbolic_registers("place_player_hud_tiles")
