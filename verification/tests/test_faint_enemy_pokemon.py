...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "FaintEnemyPokemon")
    ...
    project.hook(base + 3, Sm83LoadAImmediate(W_IS_IN_BATTLE, base + 6), length=3)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_faint_enemy_pokemon")
...
def test_faint_enemy_pokemon_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("faint_enemy_pokemon")
    values["is_in_battle"] = claripy.BVS("faint_enemy_pokemon_is_in_battle", 8)
