...
NOTHING_HAPPENED_TEXT = 0x7B3E
EXPECTED = bytes.fromhex("1afe44d0213e7bc3493c")
...
def _assembly_project() -> tuple[angr.Project, int]:
    location = symbol_location(SYMS, "CantLowerAnymore")
...
def _native_project() -> tuple[angr.Project, int]:
    ...
    assert function is not None and print_text is not None
...
def test_cant_lower_anymore_pathwise_equivalence(effect_address: int) -> None:
    values = _inputs(f"cant_lower_{effect_address:04x}", effect_address)
    assert_pathwise_equivalent(
        _assembly(values, effect_address),
        _native(values, effect_address),
        (*REGISTERS, "memory", "print_call", "trace"),
    )
