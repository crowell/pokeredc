...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "TextScriptEndingText")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_text_script_ending_text")
...
def test_text_script_ending_text_symbolic_equivalence() -> None:
    inputs = symbolic_registers("tse")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_text_script_ending_text_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "TextScriptEndingText")
    assert linked_bytes(ROM, loc, 5) == bytes.fromhex(
        "5021d624c9"
    )
