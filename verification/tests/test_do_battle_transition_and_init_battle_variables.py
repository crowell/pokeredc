...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        value = self.state.memory.load(LINK_STATE, 1)
        ...
        self.state.regs.f |= claripy.If(value.ULT(4), claripy.BVV(0x01, 8), claripy.BVV(0, 8))
        self.state.regs.f |= claripy.If(value == 4, claripy.BVV(0x40, 8), claripy.BVV(0, 8))
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "DoBattleTransitionAndInitBattleVariables")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_do_battle_transition_and_init_battle_variables")
...
def test_do_battle_transition_and_init_battle_variables_pathwise_equivalence() -> None:
    values = symbolic_registers("battle_init")
    values["link_state"] = claripy.BVS("battle_init_link_state", 8)
