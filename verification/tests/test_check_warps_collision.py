"""Proof for CheckWarpsCollision's first warp-scan setup."""
...
EXPECTED = bytes.fromhex("faaed34f21afd3")
...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    project.hook(location.address + 0x04, LoadHLImmediate(W_WARP_ENTRIES, location.address + 0x07), length=3)
...
def test_check_warps_collision_prefix_pathwise_equivalence() -> None:
    inputs = symbolic_registers("check_warps_collision")
    assert_pathwise_equivalent(_assembly(inputs), _native(inputs), REGISTERS)


def test_check_warps_collision_exact_prefix() -> None:
    location = symbol_location(SYMBOLS, "CheckWarpsCollision")
