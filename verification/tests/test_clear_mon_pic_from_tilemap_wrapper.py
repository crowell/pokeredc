...
class AddHLDE(angr.SimProcedure):
    ...
    def run(self) -> None:
        ...
        total = claripy.ZeroExt(1, hl) + claripy.ZeroExt(1, de)
        self.state.regs.f = (
            (self.state.regs.f & claripy.BVV(0x40, 8))
            | claripy.If(
                ((hl & claripy.BVV(0x0FFF, 16)) + (de & claripy.BVV(0x0FFF, 16))).UGT(0x0FFF),
                claripy.BVV(0x10, 8),
                claripy.BVV(0, 8),
            )
            | claripy.If(
                total[16] == claripy.BVV(1, 1),
                claripy.BVV(1, 8),
                claripy.BVV(0, 8),
            )
        )
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    continuation = symbol_location(SYMBOLS, "ClearScreenArea")
    ...
    project.hook(location.address + 9, ContinuationBoundary(), length=1)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_clear_mon_pic_from_tilemap_wrapper")
...
def test_clear_mon_pic_from_tilemap_wrapper_pathwise_equivalence() -> None:
    values = symbolic_registers("clear_mon_pic_from_tilemap_wrapper")
