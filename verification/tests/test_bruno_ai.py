...
def assembly(v):
 l=symbol_location(SYMS,'BrunoAI');t=symbol_location(SYMS,'AIUseXDefend');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_bruno_ai');t=p.loader.find_symbol('port_ai_use_x_defend');assert f and t
...
def test_bruno_ai_pathwise_equivalence():
 v=inputs('bruno_ai');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ai'))
