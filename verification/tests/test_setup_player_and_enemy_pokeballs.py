...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.l = claripy.BVV(0x18, 8)
        self.state.regs.b = claripy.BVV(4, 8)
        self.state.regs.f = (self.state.regs.f & 1) | claripy.BVV(0x42, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "SetupPlayerAndEnemyPokeballs")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_setup_player_and_enemy_pokeballs")
...
def test_setup_player_and_enemy_pokeballs_pathwise_equivalence() -> None:
    values = symbolic_registers("setup_both_pokeballs")
