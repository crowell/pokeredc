...
class Boundary(angr.SimProcedure):
    """The `JP 0x7b5` tail: an explicit boundary sentinel."""
...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "CheckWarpsNoCollisionRetry2")
    ...
    # `JP 0x7b5` is an explicit boundary.
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol(
        "port_check_warps_no_collision_retry2"
    )
...
def test_check_warps_no_collision_retry2_symbolic_equivalence() -> None:
    i = _inputs("cwn")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_check_warps_no_collision_retry2_exact_linked_body() -> None:
    location = symbol_location(SYMBOLS, "CheckWarpsNoCollisionRetry2")
    # INC HL / INC HL / JP 0x7b5
    assert linked_bytes(ROM, location, 5) == bytes.fromhex("2323c3b507")
