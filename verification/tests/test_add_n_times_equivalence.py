...
class AddHlBc(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        bc = self.state.regs.bc
        wide = claripy.ZeroExt(16, hl) + claripy.ZeroExt(16, bc)
        c_add = wide[16]
        self.state.regs.hl = wide[15:0]
        # ADD HL,BC: N=0, Z unchanged; only C (bit0) matters, preserved by DEC A.
        self.state.regs.f = claripy.Concat(
            claripy.BVV(0, 7), c_add
        )
        self.jump(self._n)
...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "AddNTimes")
    q = symbol_location(SYMBOLS, "AddNTimes.loop").address
    p = angr.Project(
    ...
    p.hook(q, AddHlBc(q + 1), length=1)      # add hl, bc
    p.hook(q + 1, DecA(DONE), length=1)      # dec a
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_add_n_times_step")
    ...
    return [
        E(
            **native_registers(x, NATIVE_STATE),
            memory=x.memory.load(NATIVE_STATE + 6, 2),
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "AddNTimes")
    assert linked_bytes(ROM, loc, 7) == bytes.fromhex("a7c8093d20fcc9")
