...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "GetPointerWithinSpriteStateData2")
    ...
    project.hook(
        base + 2,
        Sm83LoadAHighImmediate(0x8B, base + 4),
        length=2,
    )
    project.hook(
        base + 5,
        Sm83LoadAHighImmediate(0x8C, base + 7),
        length=2,
    )
    project.hook(base + 7, Sm83SwapRegister("a", base + 9), length=2)
    project.hook(base + 9, Sm83AddRegister("b", base + 10), length=1)
    project.hook(base + 11, ReturnBoundary(), length=1)
...
def test_get_pointer_within_sprite_state_data2_pathwise_equivalence() -> None:
    values = symbolic_registers("get_pointer_within_sprite_state_data2")
    values["sprite_data_offset"] = claripy.BVS("sprite_data_offset_2", 8)
    values["sprite_index"] = claripy.BVS("sprite_index_2", 8)
