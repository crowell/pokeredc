...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        self.state.regs.a = self.state.memory.load(SPECIES, 1)
        self.state.regs.h = claripy.BVV(0xC4, 8)
        self.state.regs.l = claripy.BVV(5, 8)
        self.state.regs.b = claripy.BVV(7, 8)
        self.state.regs.c = claripy.BVV(8, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "LoadMonBackPic")
    ...
    project.hook(location.address, Setup(), length=9)
    state = project.factory.blank_state(addr=location.address)
    set_assembly_registers(state, values)
    state.memory.store(SPECIES, values["battle_mon_species"])
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_load_mon_back_pic")
    assert function is not None
    state = project.factory.call_state(function.rebased_addr, NATIVE_STATE)
    store_native_registers(state, NATIVE_STATE, values)
    state.memory.store(NATIVE_STATE + 8, values["battle_mon_species"])
    manager = project.factory.simulation_manager(state)
    manager.run()
    assert not manager.errored
...
def test_load_mon_back_pic_setup_pathwise_equivalence() -> None:
    values = symbolic_registers("load_mon_back_pic")
    values["battle_mon_species"] = claripy.BVS("load_mon_back_pic_species", 8)
