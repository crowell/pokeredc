...
class LoadRegisterValue(angr.SimProcedure):
 def __init__(self,key,register,n,inc=0):super().__init__();self.key=key;self.register=register;self.n=n;self.inc=inc
 def run(self):setattr(self.state.regs,self.register,self.state.globals[self.key]);self.state.regs.hl=self.state.regs.hl+self.inc;self.jump(self.n)
class CopyATo(angr.SimProcedure):
 def __init__(self,register,n):super().__init__();self.register=register;self.n=n
 def run(self):setattr(self.state.regs,self.register,self.state.regs.a);self.jump(self.n)
...
def assembly_adjust_hp(i):
 p,q=project();p.hook(q+0x163,Sm83AddHlRegisterPair('bc',q+0x164),length=1);p.hook(q+0x164,LoadValue('old_max_hp_high',q+0x165,1),length=1);p.hook(q+0x166,LoadRegisterValue('old_max_hp_low','c',q+0x167),length=1);p.hook(q+0x16a,LoadValue('loaded_max_hp_low',q+0x16b,-1),length=1);p.hook(q+0x16b,Sm83SubRegister('c',q+0x16c),length=1);p.hook(q+0x16c,CopyATo('c',q+0x16d),length=1);p.hook(q+0x16d,LoadValue('loaded_max_hp_high',q+0x16e),length=1);p.hook(q+0x16e,Sm83SbcRegister('b',q+0x16f),length=1);p.hook(q+0x16f,CopyATo('b',q+0x170),length=1);p.hook(q+0x173,LoadValue('loaded_hp_low',q+0x174),length=1);p.hook(q+0x174,Sm83AddRegister('c',q+0x175),length=1);p.hook(q+0x175,StoreValue('loaded_hp_low',q+0x176,-1),length=1);p.hook(q+0x176,LoadValue('loaded_hp_high',q+0x177),length=1);p.hook(q+0x177,Sm83AdcRegister('b',q+0x178),length=1);p.hook(q+0x178,StoreValue('loaded_hp_high',q+0x179),length=1);p.hook(q+0x17a,RestoreCopyBC(DONE),length=1);s=p.factory.blank_state(addr=q+0x160);setup(s,i);return collect(p.factory.simulation_manager(s))
def assembly_done(i):
