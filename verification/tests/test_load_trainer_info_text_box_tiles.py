...
DONE = 0xEFFF
MARKER = 0x1234
EXPECTED = bytes.fromhex("11987b21609701090bc34818")


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "LoadTrainerInfoTextBoxTiles")
    ...
    for register in REGISTERS:
        state.globals[f"transfer_{register}"] = values[
            f"transfer_{register}"
        ]
...
def test_load_trainer_info_text_box_tiles_pathwise_equivalence() -> None:
    values = _inputs("load_trainer_info_text_box_tiles")
