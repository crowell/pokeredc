...
ARROW_SLOT = 0xC4F2
CLEAR_CURSOR = 0xC4A5
CLEAR_SPAN = 78  # four 18-byte rows at a 20-byte screen stride
TEXT_CURSOR = 0xC4B9
...
def _setup(state: angr.SimState, base: int,
    ...
    for offset in range(CLEAR_SPAN):
...
def _assembly(values: dict[str, claripy.ast.BV], link_state: claripy.ast.BV,
    ...
    location = symbol_location(SYMBOLS, "Paragraph")
    assert linked_bytes(ROM, location, 33) == bytes.fromhex(
        "d53eeeeaf2c4cd3a1bcd983821a5c4011204cdc4180e14cd3937d121b9c4c3e819"
    )
    ...
    project.hook(q + 12, LoadHL(CLEAR_CURSOR), length=3)
    project.hook(q + 15, LoadBC(), length=3)
    ...
    project.hook(q + 28, LoadHL(TEXT_CURSOR), length=3)
    project.hook(q + 31, ContinuationBoundary(), length=3)
...
def _native(values: dict[str, claripy.ast.BV], link_state: claripy.ast.BV,
    ...
    function = project.loader.find_symbol("port_paragraph")
...
def test_paragraph_pathwise_equivalence(link_value: int) -> None:
    values = symbolic_registers("paragraph")
    values["arrow"] = claripy.BVS("paragraph_arrow", 8)
    for offset in range(CLEAR_SPAN):
        values[f"clear{offset}"] = claripy.BVS(f"paragraph_clear_{offset}", 8)
    saved_d = claripy.BVS("paragraph_saved_d", 8)
    saved_e = claripy.BVS("paragraph_saved_e", 8)
    values["d"] = saved_d
    values["e"] = saved_e
