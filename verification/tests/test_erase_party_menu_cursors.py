...
class Store(angr.SimProcedure):
 ...
 def run(self):  # type: ignore[override]
  ...
  self.state.globals["entered"]=True;self.state.globals["written"]=claripy.BVV(0x7f,8);self.jump(self.n)
...
def begin(i):
 loc=symbol_location(SYMBOLS,"ErasePartyMenuCursors");p=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":loc.address});p.hook(loc.address+8,Bound(LOOP),length=1);s=p.factory.blank_state(addr=loc.address);set_assembly_registers(s,i);m=p.factory.simulation_manager(s);m.explore(find=LOOP);x=m.found[0];return E(**assembly_registers(x),written=i["written"],cont=claripy.BVV(1,8),constraints=tuple(x.solver.constraints))
def step(i):
 loc=symbol_location(SYMBOLS,"ErasePartyMenuCursors");loop=loc.address+8;p=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":loop});p.hook(loop,Store(loop+2),length=2);p.hook(loop+2,Sm83AddHlRegisterPair("bc",loop+3),length=1);p.hook(loop+3,Sm83DecRegister("a",loop+4),length=1);p.hook(loop+6,Bound(RETURN),length=1);s=p.factory.blank_state(addr=loop);set_assembly_registers(s,i);s.globals["written"]=i["written"];m=p.factory.simulation_manager(s);m.stashes["found"]=[]
 while m.active:
...
def test_begin():
 i=inputs("erase_party_cursors_begin");assert_pathwise_equivalent([begin(i)],native("port_erase_party_menu_cursors_begin",i),(*REGISTERS,"cont"))
@pytest.mark.skipif(not NATIVE_ELF.exists(),reason="native")
def test_step():
 i=inputs("erase_party_cursors_step");assert_pathwise_equivalent(step(i),native("port_erase_party_menu_cursors_step",i,True),(*REGISTERS,"written","cont"))
def test_body():
 loc=symbol_location(SYMBOLS,"ErasePartyMenuCursors");assert linked_bytes(ROM,loc,15)==bytes.fromhex("21b4c30128003e06367f093d20fac9")
