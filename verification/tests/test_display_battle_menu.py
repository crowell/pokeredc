...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "DisplayBattleMenu")
    ...
    project.hook(base + 3, Sm83LoadAImmediate(BATTLE_TYPE, base + 6), length=3)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_display_battle_menu")
...
def test_display_battle_menu_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("display_battle_menu")
    values["battle_type"] = claripy.BVS("display_battle_menu_battle_type", 8)
