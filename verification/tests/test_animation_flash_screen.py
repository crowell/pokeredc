...
class FlashSummary(angr.SimProcedure):
    def run(self) -> None:
        palette = self.state.memory.load(R_BGP, 1)
        self.state.regs.a = palette
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    project.hook(location.address, FlashSummary(), length=1)
