...
from verification.tests.test_text_script_bills_pc import HoldBoundary, SaveBoundary
...
SCREEN_AREA = 0x168
PLAYER_BANK = 1
PLAYER_POINTER = 0x78E6
RETURN_H = 0x35
RETURN_L = 0xE4
EXPECTED = bytes.fromhex("cdf436060121e678180f")
...
class BankswitchBoundary(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        state.globals["callback_call"] = claripy.Concat(
            claripy.BVV(PLAYER_BANK, 8), z80_flags_to_sm83(state.regs.f),
            claripy.BVV(RETURN_H, 8), claripy.BVV(RETURN_L, 8),
            state.regs.d, state.regs.e,
            claripy.BVV(PLAYER_POINTER >> 8, 8),
            claripy.BVV(PLAYER_POINTER & 0xff, 8),
            claripy.BVV(PLAYER_BANK, 8), claripy.BVV(PLAYER_BANK, 8))
    ...
    return Endpoint(
        **(native_registers(state, NATIVE_STATE)
        ...
        memory=(claripy.Concat(
            state.memory.load(H_LOADED, 1), state.memory.load(R_ROMB, 1),
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "TextScript_ItemStoragePC")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_text_script_item_storage_pc")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(),
                    reason="run `make red`")
def test_text_script_item_storage_pc_pathwise_equivalence() -> None:
    values = symbolic_registers("item_storage_pc")
    values["loaded"] = claripy.BVS("item_storage_pc_loaded", 8)
    values["romb"] = claripy.BVS("item_storage_pc_romb", 8)
    values["tilemap"] = [claripy.BVS(f"item_storage_pc_tile_{i}", 8)
                          for i in range(SCREEN_AREA)]
    for name in REGISTERS:
        values[f"callback_{name}"] = (
            claripy.Concat(claripy.BVS("item_storage_pc_callback_flags", 4),
                           claripy.BVV(0, 4))
            if name == "f" else
            claripy.BVS(f"item_storage_pc_callback_{name}", 8))
    values["callback_loaded"] = claripy.BVS("item_storage_pc_callback_loaded", 8)
    values["callback_mapper"] = claripy.BVS("item_storage_pc_callback_mapper", 8)
    for name in REGISTERS:
        values[f"out_{name}"] = (
            claripy.Concat(claripy.BVS("item_storage_pc_out_flags", 4),
                           claripy.BVV(0, 4))
            if name == "f" else
            claripy.BVS(f"item_storage_pc_out_{name}", 8))
