...
EXPECTED_ENTRY = bytes.fromhex("016f6f1821")


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    entry = symbol_location(SYMBOLS, "AnimationDarkScreenPalette")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol(
        "port_animation_dark_screen_palette_player"
    )
...
def test_animation_dark_screen_palette_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_dark_screen_palette")
    values["on_sgb"] = claripy.BVS("animation_dark_screen_palette_on_sgb", 8)
