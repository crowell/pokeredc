...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "GBFadeInFromWhite")
    ...
    for offset, hardware_offset in ((6, 0x49), (9, 0x48), (12, 0x47)):
        project.hook(
            base + offset,
            Sm83StoreAHighImmediate(hardware_offset, base + offset + 2),
            length=2,
        )
    project.hook(base + 16, DelayFramesTerminal(base + 19), length=3)
    project.hook(base + 19, Sm83DecRegister("b", base + 20), length=1)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    palette_start = symbol_location(SYMBOLS, "FadePal5")
    palette_bytes = linked_bytes(ROM, palette_start, 9)
    project = angr.Project(NATIVE_ELF, auto_load_libs=False)
    function = project.loader.find_symbol("port_gb_fade_in_from_white")
    assert function is not None
    state = project.factory.call_state(
        function.rebased_addr, NATIVE_STATE, NATIVE_MEMORY
    )
    store_native_registers(state, NATIVE_STATE, values)
    state.memory.store(NATIVE_MEMORY + palette_start.address, palette_bytes)
    for address in PALETTE_REGISTERS:
        state.memory.store(
...
def test_gb_fade_in_from_white_pathwise_equivalence() -> None:
    values = symbolic_registers("gb_fade_in_from_white")
    for address in PALETTE_REGISTERS:
        values[f"palette_{address:04x}"] = claripy.BVS(
            f"gb_fade_in_from_white_palette_{address:04x}", 8
        )
