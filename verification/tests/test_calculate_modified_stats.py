...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "CalculateModifiedStats")
    ...
    project.hook(location.address, Setup(), length=2)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_calculate_modified_stats")
...
def test_calculate_modified_stats_setup_pathwise_equivalence() -> None:
    values = symbolic_registers("calculate_modified_stats")
