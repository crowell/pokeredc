...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "DrainHPEffect")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_drain_hp_effect")
...
def test_drain_hp_effect_symbolic_equivalence() -> None:
    inputs = symbolic_registers("dhp")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_drain_hp_effect_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "DrainHPEffect")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("213f780601c3d635")
