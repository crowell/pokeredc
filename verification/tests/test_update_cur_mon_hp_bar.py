...
class SetupHL(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.l = claripy.BVV(0x5E, 8)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "UpdateCurMonHPBar")
    ...
    project.hook(base + 6, LoadAOne(), length=2)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_update_cur_mon_hp_bar")
...
def test_update_cur_mon_hp_bar_setup_pathwise_equivalence() -> None:
    values = symbolic_registers("update_cur_mon_hp_bar")
    values["whose_turn"] = claripy.BVS("update_cur_mon_hp_bar_whose_turn", 8)
