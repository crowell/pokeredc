...
class Boundary(angr.SimProcedure):
    """The tail `jp` to the shared CopyTilemap routine is the boundary."""
...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "LoadBattleTransitionTile")
    base = loc.address
...
def test_load_battle_transition_tile_symbolic_equivalence() -> None:
    inputs = symbolic_registers("lbtt")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_load_battle_transition_tile_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "LoadBattleTransitionTile")
    assert linked_bytes(ROM, loc, 12) == bytes.fromhex(
        "21f08f11594a01011cc34818"
    )
