...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "CopyToRedrawRowOrColumnSrcTiles")
    q = symbol_location(SYMBOLS, "CopyToRedrawRowOrColumnSrcTiles.loop").address
    p = angr.Project(
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_copy_to_redraw_src_tiles_step")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "CopyToRedrawRowOrColumnSrcTiles")
    assert linked_bytes(ROM, loc, 12) == bytes.fromhex("11fccb0e282a12130d20fac9")
