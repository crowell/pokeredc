...
def assembly(v):
 l=symbol_location(SYMS,'CeladonMart2F_Script');t=symbol_location(SYMS,'EnableAutoTextBoxDrawing');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_celadon_mart2_f_script');t=p.loader.find_symbol('port_enable_auto_text_box_drawing');assert f and t
...
def test_celadon_mart2_f_script_pathwise_equivalence():
 v=inputs('celadon_mart2_f_script');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'control','wait','call'))
