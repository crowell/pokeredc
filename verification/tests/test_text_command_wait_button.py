...
class ManualTextScrollSite(angr.SimProcedure):
 ...
 its terminating normal A/B or link-battle delay observation (the
 established text-poll boundary precedent): the saved registers pass
 through on the normal path, while link battle returns A := 4, C := 65,
 and canonical N|Z flags."""
...
def assembly(v):
 ...
 p.hook(b+0x01,ManualTextScrollSite(b+0x04,link=v['link'].args[0] == 4),length=3)
