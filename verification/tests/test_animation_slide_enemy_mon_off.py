...
DONE = 0xEFFF
ANIMATION_SLIDE_MON_OFF = 0x52AF
...
class LoadSlideMonOffPointer(angr.SimProcedure):
    def run(self) -> None:
        self.inhibit_autoret = True
        state = self.state.copy()
        state.regs.h = claripy.BVV(ANIMATION_SLIDE_MON_OFF >> 8, 8)
        state.regs.l = claripy.BVV(ANIMATION_SLIDE_MON_OFF & 0xFF, 8)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "AnimationSlideEnemyMonOff")
    ...
    project.hook(location.address, LoadSlideMonOffPointer(), length=3)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_animation_slide_enemy_mon_off")
...
def test_animation_slide_enemy_mon_off_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_slide_enemy_mon_off")
