...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "GBFadeOutToWhite")
    ...
    for offset, hardware_offset in ((6, 0x47), (9, 0x48), (12, 0x49)):
        project.hook(
            base + offset,
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "FadePal6")
    palette_bytes = linked_bytes(ROM, location, 9)
    project = angr.Project(NATIVE_ELF, auto_load_libs=False)
    function = project.loader.find_symbol("port_gb_fade_out_to_white")
...
def test_gb_fade_out_to_white_pathwise_equivalence() -> None:
    values = symbolic_registers("gb_fade_out_to_white")
    for address in PALETTE_REGISTERS:
        values[f"palette_{address:04x}"] = claripy.BVS(
            f"gb_fade_out_to_white_palette_{address:04x}", 8
        )
