...
def _assembly(values: dict[str, claripy.ast.BV]):
    location = symbol_location(SYMBOLS, "TMChar")
    target = symbol_location(SYMBOLS, "PlaceCommandCharacter")
    expected = bytes.fromhex("d511551a183a")
...
def _native(values: dict[str, claripy.ast.BV]):
    ...
    function = project.loader.find_symbol("port_tm_char")
...
def test_tm_char_pathwise_equivalence() -> None:
    values = symbolic_registers("tm_char")
