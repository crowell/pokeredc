...
HANDLER_EXPECTED = bytes.fromhex(
    "e12a57e560693e7522d5cd9a01d1f0b4e60320050e0acd39371520ea444de1c3551b"
)
...
def _inputs(prefix: str) -> dict[str, claripy.ast.BV]:
    ...
    # The text pointer (HL on entry) and the destination (B:C) are opaque to
    # the handler: it only reads the dot count at [HL] and writes ellipsis
    # tiles at B:C. Fixed RAM addresses keep the native stores from forking on
    # symbolic pointers while every other input stays symbolic.
    values["pushed_hl"] = claripy.BVV(TEXT_PTR, 16)
    values["b"] = claripy.BVV(TEXT_DEST >> 8, 8)
    values["c"] = claripy.BVV(TEXT_DEST & 0xFF, 8)
...
def _setup(
    ...
    # The dot count byte lives at the fixed text pointer.
    state.memory.store(base + TEXT_PTR, claripy.BVV(count, 8))


def _endpoint(
    state: angr.SimState,
    native: bool,
    dest_base: claripy.ast.BV,
    count: int,
) -> Endpoint:
    base = NATIVE_MEMORY if native else 0
    registers = (
    ...
    return Endpoint(
        **registers,
        ...
        window=claripy.Concat(
            *(
                state.memory.load(base + claripy.ZeroExt(16, dest_base) + i, 1)
...
class PushHL(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.memory.store(sp - 1, self.state.regs.h)
        self.state.memory.store(sp - 2, self.state.regs.l)
...
class StoreHLInc(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        addr = claripy.Concat(self.state.regs.h, self.state.regs.l)
        self.state.memory.store(addr, self.state.regs.a)
        addr = addr + 1
        self.state.regs.h = addr[15:8]
        self.state.regs.l = addr[7:0]
        self.jump(self.next_address)
...
class NativeJoypadBoundary(angr.SimProcedure):
    """Native-side mirror of JoypadBoundary, reusing the same proved callee
    transition on the composed register/memory model."""

    def run(self) -> None:  # type: ignore[override]
        memory = self.state.memory
        input_value = memory.load(NATIVE_MEMORY + H_JOYINPUT, 1)
        last = memory.load(NATIVE_MEMORY + H_JOYLAST, 1)
        ignore = memory.load(NATIVE_MEMORY + W_JOYIGNORE, 1)
        disabled = (
            memory.load(NATIVE_MEMORY + W_STATUSFLAGS5, 1)
            & (1 << BIT_DISABLE_JOYPAD)
        ) != 0
        normal = input_value != PAD_BUTTONS
        unmasked = claripy.Or(disabled, ignore == 0)
        mask = ~ignore
        ...
        saved_a = memory.load(NATIVE_MEMORY + H_LOADED_ROM_BANK, 1)
        saved_f = memory.load(NATIVE_STATE + 1, 1)
        memory.store(
            NATIVE_MEMORY + H_JOYLAST,
            claripy.If(normal, input_value, memory.load(NATIVE_MEMORY + H_JOYLAST, 1)),
        )
        memory.store(
            NATIVE_MEMORY + H_JOYRELEASED,
            claripy.If(
                normal,
                released_after,
                memory.load(NATIVE_MEMORY + H_JOYRELEASED, 1),
            ),
        )
        memory.store(
            NATIVE_MEMORY + H_JOYPRESSED,
            claripy.If(
                normal,
                pressed_after,
                memory.load(NATIVE_MEMORY + H_JOYPRESSED, 1),
            ),
        )
        memory.store(
            NATIVE_MEMORY + H_JOYHELD,
            claripy.If(normal, held_after, memory.load(NATIVE_MEMORY + H_JOYHELD, 1)),
        )
        b = memory.load(NATIVE_STATE + 2, 1)
        d = memory.load(NATIVE_STATE + 4, 1)
        e = memory.load(NATIVE_STATE + 5, 1)
        memory.store(
            NATIVE_STATE + 2,
            claripy.If(normal, claripy.If(unmasked, input_value, mask), b),
        )
        memory.store(
            NATIVE_STATE + 4,
            claripy.If(normal, input_value ^ last, d),
        )
        memory.store(
            NATIVE_STATE + 5,
            claripy.If(normal, last, e),
        )
        memory.store(NATIVE_STATE + 0, saved_a)
        memory.store(NATIVE_STATE + 1, saved_f)
        memory.store(NATIVE_MEMORY + H_LOADED_ROM_BANK, saved_a)
        memory.store(NATIVE_MEMORY + R_ROMB, saved_a)


class NativeDelayFrameBoundary(angr.SimProcedure):
    """Native-side mirror of DelayFramesBoundary: one proved DelayFrame
    transition, terminating the DelayFrames loop on its concrete count."""

    def run(self, delay_state: claripy.ast.BV, _observations: claripy.ast.BV) -> None:  # type: ignore[override]
        self.state.globals["delay_frame_calls"] += 1
        self.state.memory.store(delay_state, claripy.BVV(0, 8))
        self.state.memory.store(delay_state + 1, claripy.BVV(0x50, 8))


def _assembly(values: dict[str, claripy.ast.BV], count: int) -> list[Endpoint]:
    handler = symbol_location(SYMBOLS, "TextCommand_DOTS")
    assert handler.bank == 0
    assert handler.address == 0x1C78
    ...
    project.hook(base + 1, Sm83LoadAAtHlIncrement(base + 2), length=1)
    project.hook(base + 2, CopyRegister("d", "a", base + 3), length=1)
    project.hook(base + 3, PushHL(base + 4), length=1)
    project.hook(base + 4, CopyRegister("h", "b", base + 5), length=1)
    project.hook(base + 5, CopyRegister("l", "c", base + 6), length=1)
    project.hook(base + 6, LoadAImmediate(0x75, base + 8), length=2)
    project.hook(base + 8, StoreHLInc(base + 9), length=1)
    project.hook(base + 9, PushDE(base + 10), length=1)
    project.hook(base + 10, JoypadBoundary(base + 13), length=3)
    project.hook(base + 13, PopDE(base + 14), length=1)
    project.hook(
        base + 14, Sm83LoadAHighImmediate(0xB4, base + 16), length=2
    )
    project.hook(
        base + 16, Sm83AndImmediateCorrect(0x03, base + 18), length=2
    )
    project.hook(
        base + 18,
        ForkOnZ(base + 25, base + 20, taken_when_set=False),
        length=2,
    )
    project.hook(base + 20, LoadCImmediate(10, base + 22), length=2)
    project.hook(base + 22, DelayFramesBoundary(base + 25), length=3)
    project.hook(base + 25, Sm83DecRegister("d", base + 26), length=1)
    project.hook(
        base + 26,
        ForkOnZ(base + 6, base + 28, taken_when_set=False),
        length=2,
    )
    project.hook(base + 28, CopyRegister("b", "h", base + 29), length=1)
    project.hook(base + 29, CopyRegister("c", "l", base + 30), length=1)
    project.hook(base + 30, PopHL(base + 31), length=1)
    project.hook(base + 31, Jump(CONTINUATION), length=3)

    state = project.factory.blank_state(addr=base)
    set_assembly_registers(state, values)
    _setup(state, values, native=False, count=count)
    ...
    manager.explore(find=CONTINUATION, num_find=1 << (count + 1))
    if manager.errored:
        for es in manager.errored:
            try:
                print("ASM ERRORED ip=", hex(int(es.state.solver.eval(es.state.regs.ip))), "err=", es.error)
            except Exception as exc:
                print("ASM ERRORED (no state):", es.error, exc)
    assert len(manager.found) == 2
    assert not manager.errored
    dest_base = claripy.Concat(values["b"], values["c"])
    return [
        _endpoint(final, native=False, dest_base=dest_base, count=count)
        for final in manager.found
    ]


def _native(values: dict[str, claripy.ast.BV], count: int) -> list[Endpoint]:
    project = angr.Project(NATIVE_ELF, auto_load_libs=False)
    function = project.loader.find_symbol("port_text_command_dots")
    joypad = project.loader.find_symbol("port_joypad_homecall")
    delay_frame = project.loader.find_symbol("port_delay_frames")
    assert function is not None and joypad is not None and delay_frame is not None
    project.hook(joypad.rebased_addr, NativeJoypadBoundary())
    project.hook(delay_frame.rebased_addr, NativeDelayFrameBoundary())
    state = project.factory.call_state(
        function.rebased_addr, NATIVE_STATE, NATIVE_MEMORY
    )
    store_native_registers(state, NATIVE_STATE, values)
    _setup(state, values, native=True, count=count)
    state.globals["delay_frame_calls"] = claripy.BVV(0, 8)
    state.memory.store(NATIVE_STATE + 6, values["pushed_hl"][15:8])
    state.memory.store(NATIVE_STATE + 7, values["pushed_hl"][7:0])
    manager = project.factory.simulation_manager(state)
    manager.run()
    assert len(manager.deadended) == 2
    dest_base = claripy.Concat(values["b"], values["c"])
    return [
        _endpoint(final, native=True, dest_base=dest_base, count=count)
...
@pytest.mark.parametrize("count", (1, 2, 3, 4), ids=lambda v: str(v))
def test_text_command_dots_pathwise_equivalence(count: int) -> None:
    values = _inputs(f"text_command_dots_{count}")
    assert_pathwise_equivalent(
        _assembly(values, count),
        _native(values, count),
        (
            *REGISTERS,
            "joy_input",
            "joy_last",
