...
def assembly(v):
 l=symbol_location(SYMS,'AnimationShowEnemyMonPic');t=symbol_location(SYMS,'CallWithTurnFlipped');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_animation_show_enemy_mon_pic');t=p.loader.find_symbol('port_call_with_turn_flipped');assert f and t
...
def test_animation_show_enemy_mon_pic_pathwise_equivalence():
 v=inputs('animation_show_enemy_mon_pic');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'cwt'))
