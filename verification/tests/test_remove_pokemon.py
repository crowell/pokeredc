...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "RemovePokemon")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_remove_pokemon")
...
def test_remove_pokemon_symbolic_equivalence() -> None:
    inputs = symbolic_registers("rp")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_remove_pokemon_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "RemovePokemon")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("21687b0601c3d635")
