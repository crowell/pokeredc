...
EXPECTED=bytes.fromhex('fe40d0c3{:02x}{:02x}'.format(symbol_location(SYMS,'AIUseXAttack').address&0xff,symbol_location(SYMS,'AIUseXAttack').address>>8))
...
def assembly(v):
 l=symbol_location(SYMS,'CooltrainerMAI');t=symbol_location(SYMS,'AIUseXAttack');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_cooltrainer_m_ai');t=p.loader.find_symbol('port_ai_use_x_attack');assert f and t
...
def test_cooltrainer_m_ai_pathwise_equivalence():
 v=inputs('cooltrainer_m_ai');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ai'))
