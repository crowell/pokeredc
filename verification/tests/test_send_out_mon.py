...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "SendOutMon")
    ...
    project.hook(base + 3, SetupHL(), length=3)
    project.hook(base + 6, Sm83LoadAAtHlIncrement(base + 7), length=1)
    project.hook(base + 7, OrAtHL(), length=1)
    project.hook(base + 8, Boundary(), length=3)
    state = project.factory.blank_state(addr=base)
    set_assembly_registers(state, values)
    state.memory.store(HP, values["enemy_hp_low"])
    state.memory.store(HP + 1, values["enemy_hp_high"])
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_send_out_mon")
    assert function is not None
    state = project.factory.call_state(function.rebased_addr, NATIVE_STATE)
    store_native_registers(state, NATIVE_STATE, values)
    state.memory.store(NATIVE_STATE + 8, values["enemy_hp_low"])
    state.memory.store(NATIVE_STATE + 9, values["enemy_hp_high"])
    manager = project.factory.simulation_manager(state)
    manager.run()
    assert not manager.errored
    return [
...
def test_send_out_mon_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("send_out_mon")
    values["enemy_hp_low"] = claripy.BVS("send_out_mon_enemy_hp_low", 8)
    values["enemy_hp_high"] = claripy.BVS("send_out_mon_enemy_hp_high", 8)
