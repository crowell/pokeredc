...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "ClearScreenArea")
    q = symbol_location(SYMBOLS, "ClearScreenArea.loopTiles").address
    p = angr.Project(
    ...
    p.hook(q + 1, DecC(DONE), length=1)      # dec c
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_clear_screen_area_tile_step")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "ClearScreenArea")
    assert linked_bytes(ROM, loc, 18) == bytes.fromhex("3e7f111400e5c5220d20fcc1e1190520f4c9")
