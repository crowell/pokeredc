...
RETURN = 0xFFFF
SPRITE_INDEX = 0xFF8C
SPRITE_MOVEMENT_BYTE2 = 0xFF8D
SPRITE_STATE_DATA2 = 0xC200
MAP_SPRITE_DATA = 0xD4E4
MAP_SPRITE_DATA_SIZE = 511
EXPECTED_BODY = bytes.fromhex("e5cd4e3536fecd5835f08d77e1c9")
...
def _memory(state: angr.SimState, base: int = 0) -> claripy.ast.BV:
    return claripy.Concat(
        ...
        state.memory.load(base + SPRITE_MOVEMENT_BYTE2, 1),
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "SetSpriteMovementBytesToFE")
    ...
    project.hook(
        location.address + 9,
        Sm83LoadAHighImmediate(0x8D, location.address + 11),
        length=2,
    )
    state = project.factory.blank_state(addr=location.address)
    set_assembly_registers(state, values)
    state.memory.store(SPRITE_INDEX, values["sprite_index"])
    state.memory.store(SPRITE_MOVEMENT_BYTE2, values["movement_byte2"])
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_set_sprite_movement_bytes_to_fe")
    ...
    state.memory.store(
        NATIVE_MEMORY + SPRITE_MOVEMENT_BYTE2, values["movement_byte2"]
    )
    state.memory.store(
...
def test_set_sprite_movement_bytes_to_fe_pathwise_equivalence() -> None:
    values = symbolic_registers("set_sprite_movement_bytes_to_fe")
    values["sprite_index"] = claripy.BVS("sprite_index", 8)
    values["movement_byte2"] = claripy.BVS("movement_byte2", 8)
