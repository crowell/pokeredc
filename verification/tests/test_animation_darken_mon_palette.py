...
EXPECTED_ENTRY = bytes.fromhex("01f4f9181c")


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    entry = symbol_location(SYMBOLS, "AnimationDarkenMonPalette")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol(
        "port_animation_darken_mon_palette_player"
    )
...
def test_animation_darken_mon_palette_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_darken_mon_palette")
    values["on_sgb"] = claripy.BVS("animation_darken_mon_palette_on_sgb", 8)
