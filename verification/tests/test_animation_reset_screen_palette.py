...
EXPECTED_ENTRY = bytes.fromhex("01e4e4180d")


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    entry = symbol_location(SYMBOLS, "AnimationResetScreenPalette")
...
def test_animation_reset_screen_palette_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_reset_screen_palette")
    values["on_sgb"] = claripy.BVS("animation_reset_screen_palette_on_sgb", 8)
    values["palette"] = claripy.BVS("animation_reset_screen_palette_value", 8)
