...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    continuation = symbol_location(SYMBOLS, "GetTileIDList")
    ...
    project.hook(location.address, XorA(), length=1)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    project = angr.Project(NATIVE_ELF, auto_load_libs=False); function = project.loader.find_symbol("port_animation_show_mon_pic"); assert function is not None
...
def test_animation_show_mon_pic_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_show_mon_pic")
