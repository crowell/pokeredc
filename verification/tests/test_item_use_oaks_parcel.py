...
def assembly(v):
 l=symbol_location(SYMS,'ItemUseOaksParcel');t=symbol_location(SYMS,'ItemUseNotYoursToUse');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_item_use_oaks_parcel');t=p.loader.find_symbol('port_item_use_not_yours_to_use');assert f and t
...
def test_item_use_oaks_parcel_pathwise_equivalence():
 v=inputs('item_use_oaks_parcel');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'tse'))
