...
class CompareHLWithBC(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        b = self.state.regs.b
        c = self.state.regs.c
        # ld a,h / sub b / ret nz: if h != b, a = h-b with sub flags.
        # else ld a,l / sub c: a = l-c with sub flags.
        diff_hb = h - b
        diff_lc = l - c
        high_equal = h == b
        self.state.regs.a = claripy.If(high_equal, diff_lc, diff_hb)
        f_z80 = claripy.If(
            high_equal,
            sm83_flags_to_z80(sm83_sub(l, c)),
            sm83_flags_to_z80(sm83_sub(h, b)),
        )
...
def inputs() -> dict:
    return symbolic_registers("hl")


def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "CompareHLWithBC")
    # ld a,h / sub b / ret nz / ld a,l / sub c / ret  (6 bytes, bank 3).
    q = loc.address
    p = angr.Project(
    ...
    p.hook(q, CompareHLWithBC(DONE), length=6)
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_compare_hl_with_bc")
...
def test_transition_equivalence() -> None:
    i = inputs()
    assert_pathwise_equivalent(assembly(i), native(i), ("a", "f", "b", "c", "h", "l"))


@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "CompareHLWithBC")
    assert linked_bytes(ROM, loc, 6) == bytes.fromhex("7c90c07d91c9")
