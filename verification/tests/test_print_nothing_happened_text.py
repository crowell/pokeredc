...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        self.state.regs.h = claripy.BVV(0x7B, 8)
        self.state.regs.l = claripy.BVV(0x3E, 8)
        self.state.regs.b = claripy.BVV(0xC4, 8)
        self.state.regs.c = claripy.BVV(0xB9, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "PrintNothingHappenedText")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_print_nothing_happened_text")
...
def test_print_nothing_happened_text_pathwise_equivalence() -> None:
    values = symbolic_registers("print_nothing")
