...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "GBFadeOutToBlack")
    ...
    for offset, hardware_offset in ((13, 0x49), (16, 0x48), (19, 0x47)):
        project.hook(
            base + offset,
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    palette_start = symbol_location(SYMBOLS, "FadePal1")
    palette_bytes = linked_bytes(ROM, palette_start, 12)
    project = angr.Project(NATIVE_ELF, auto_load_libs=False)
    function = project.loader.find_symbol("port_gb_fade_out_to_black")
...
def test_gb_fade_out_to_black_pathwise_equivalence() -> None:
    values = symbolic_registers("gb_fade_out_to_black")
    for address in PALETTE_REGISTERS:
        values[f"palette_{address:04x}"] = claripy.BVS(
            f"gb_fade_out_to_black_palette_{address:04x}", 8
        )
