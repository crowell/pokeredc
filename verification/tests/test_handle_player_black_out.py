...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "HandlePlayerBlackOut")
    ...
    project.hook(base, Sm83LoadAImmediate(W_LINK_STATE, base + 3), length=3)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_handle_player_black_out")
...
def test_handle_player_black_out_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("handle_player_black_out")
    values["link_state"] = claripy.BVS("handle_player_black_out_link_state", 8)
