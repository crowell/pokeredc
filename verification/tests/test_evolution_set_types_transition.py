...
class SetTypesBoundary(angr.SimProcedure):
    def run(self) -> None:
        self.state.globals["set_types_called"] = claripy.BVV(1, 8)
        self.jump(DONE)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    base = location.address + 0x18B
    project.hook(base, PopSavedEntry(), length=1)
    project.hook(base + 3, SetTypesBoundary(), length=3)
    project.hook(base + 6, Boundary(), length=2)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_evolution_set_types_transition")
...
def test_evolution_set_types_transition_pathwise_equivalence() -> None:
    values = _inputs("set_types")
