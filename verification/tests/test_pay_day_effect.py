...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "PayDayEffect")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_pay_day_effect")
...
def test_pay_day_effect_symbolic_equivalence() -> None:
    inputs = symbolic_registers("pde")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_pay_day_effect_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "PayDayEffect")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("21b87e060bc3d635")
