...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.d = claripy.BVV(0xD8, 8)
        self.state.regs.e = claripy.BVV(0x9C, 8)
        self.state.regs.b = claripy.BVV(4, 8)
        self.state.regs.c = claripy.BVV(0x0E, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "DrawEnemyPokeballs")
    ...
    project.hook(location.address, Setup(), length=6)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_draw_enemy_pokeballs")
...
def test_draw_enemy_pokeballs_pathwise_equivalence() -> None:
    values = symbolic_registers("draw_enemy_pokeballs")
