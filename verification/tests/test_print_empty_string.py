...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        self.state.regs.h = claripy.BVV(0x6E, 8)
        self.state.regs.l = claripy.BVV(0x9A, 8)
        self.state.regs.b = claripy.BVV(0xC4, 8)
        self.state.regs.c = claripy.BVV(0xB9, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "PrintEmptyString")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_print_empty_string")
...
def test_print_empty_string_pathwise_equivalence() -> None:
    values = symbolic_registers("print_empty_string")
