...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.l = claripy.If(whose == 0, claripy.BVV(0x1A, 8), claripy.BVV(0x2E, 8))
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "StatModifierUpEffect")
    ...
    project.hook(location.address, Setup(), length=0x11)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_stat_modifier_up_effect")
...
def test_stat_modifier_up_effect_pathwise_equivalence() -> None:
    values = symbolic_registers("stat_modifier")
    values["whose_turn"] = claripy.BVS("stat_modifier_whose", 8)
