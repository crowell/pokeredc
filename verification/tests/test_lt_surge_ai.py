...
EXPECTED=bytes.fromhex('fe40d0c3{:02x}{:02x}'.format(symbol_location(SYMS,'AIUseXSpeed').address&0xff,symbol_location(SYMS,'AIUseXSpeed').address>>8))
...
def assembly(v):
 l=symbol_location(SYMS,'LtSurgeAI');t=symbol_location(SYMS,'AIUseXSpeed');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_lt_surge_ai');t=p.loader.find_symbol('port_ai_use_x_speed');assert f and t
...
def test_lt_surge_ai_pathwise_equivalence():
 v=inputs('lt_surge_ai');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ai'))
