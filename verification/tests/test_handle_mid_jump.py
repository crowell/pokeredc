...
class Boundary(angr.SimProcedure):
    """The tail `jp` to the shared map routine is the boundary."""
...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "HandleMidJump")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_handle_mid_jump")
...
def test_handle_mid_jump_symbolic_equivalence() -> None:
    inputs = symbolic_registers("hmj")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_handle_mid_jump_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "HandleMidJump")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex(
        "061c217e48c3d635"
    )
