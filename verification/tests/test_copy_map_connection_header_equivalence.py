...
# copy_byte_step_state: registers@0 (8 bytes), fetched@+8, written@+9.
FETCHED_OFF = 8
WRITTEN_OFF = 9
...
class DecC(angr.SimProcedure):
    """Models `dec c` and the resulting flag set (the native step does
    f = (f & PORT_FLAG_C) | PORT_FLAG_N | Z(if c==0) | H(if old_c&0xf==0)).

    Packed into the shim-Z80 layout so assembly_registers converts it to the
    native raw F. result = (c == 0).
    """
...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "CopyMapConnectionHeader")
    q = loc.address + 2  # skip `ld c, 0xb`; start at the .loop body
    p = angr.Project(
    ...
    p.hook(q + 1, Store(q + 2), length=1)    # ld [de], a
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_copy_map_connection_header_step")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "CopyMapConnectionHeader")
    assert linked_bytes(ROM, loc, 9) == bytes.fromhex("0e0b2a12130d20fac9")
