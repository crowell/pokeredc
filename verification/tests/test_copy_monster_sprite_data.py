...
class LoadA5PreserveFlags(angr.SimProcedure):
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    continuation = symbol_location(SYMBOLS, "FarCopyData2")
    ...
    project.hook(location.address + 3, LoadA5PreserveFlags(location.address + 5), length=2)
...
def test_copy_monster_sprite_data_prefix_pathwise_equivalence() -> None:
    values = symbolic_registers("copy_monster_sprite_data")
