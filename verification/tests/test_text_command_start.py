...
TEXTPTR=0xd360;DEST=0xc4e1;STRLEN=5;CONT=0x1b55;HANDLER=0x1b8a;PLACESTRING=0x1955
HANDLER_EXPECTED=bytes.fromhex('e1545d6069cd5519626b2318be')
...
def inputs(p):
 ...
 for i in range(len(STRING)):v[f'str{i}']=claripy.BVV(STRING[i],8)
 for i in range(STRLEN+2):v[f'win{i}']=claripy.BVS(f'{p}_win{i}',8)
 return v
def setup(s,v,native:bool):
 o=NM if native else 0
 for i in range(len(STRING)):s.memory.store(o+TEXTPTR+i,v[f'str{i}'])
...
class LdDFromH(angr.SimProcedure):
 """SM83 LD D,H: D := the real H register (the p-code regs.h byte)."""
 def __init__(self,n:int)->None:
  super().__init__();self._n=n
 def run(self):
  self.state.regs.d=self.state.regs.h;self.jump(self._n)
class LdEFromL(angr.SimProcedure):
 """SM83 LD E,L: E := the real L register (the p-code regs.l byte)."""
 def __init__(self,n:int)->None:
  super().__init__();self._n=n
 def run(self):
  self.state.regs.e=self.state.regs.l;self.jump(self._n)
class LdHFromB(angr.SimProcedure):
 """SM83 LD H,B: the real H register := B (the p-code regs.h byte)."""
 def __init__(self,n:int)->None:
  super().__init__();self._n=n
 def run(self):
  self.state.regs.h=self.state.regs.b;self.jump(self._n)
class LdLFromC(angr.SimProcedure):
 """SM83 LD L,C: the real L register := C (the p-code regs.l byte)."""
 def __init__(self,n:int)->None:
  super().__init__();self._n=n
 def run(self):
  self.state.regs.l=self.state.regs.c;self.jump(self._n)
class LdHFromD(angr.SimProcedure):
 """SM83 LD H,D: the real H register := D (the p-code regs.h byte)."""
 def __init__(self,n:int)->None:
  super().__init__();self._n=n
 def run(self):
  self.state.regs.h=self.state.regs.d;self.jump(self._n)
class LdLFromE(angr.SimProcedure):
 """SM83 LD L,E: the real L register := E (the p-code regs.l byte)."""
 def __init__(self,n:int)->None:
  super().__init__();self._n=n
 def run(self):
  self.state.regs.l=self.state.regs.e;self.jump(self._n)
class IncHL(angr.SimProcedure):
 """SM83 INC HL: 16-bit increment of the real HL = (regs.h << 8) | regs.l."""
 def __init__(self,n:int)->None:
  super().__init__();self._n=n
 def run(self):
  cur=claripy.Concat(self.state.regs.h,self.state.regs.l)
  nxt=cur+1
  self.state.regs.h=nxt[15:8];self.state.regs.l=nxt[7:0];self.jump(self._n)
...
class PlaceStringSite(angr.SimProcedure):
 ...
 def run(self):
  ...
  self.jump(self._n)
def assembly(v):
 l=symbol_location(SYMS,'TextCommand_START')
 ...
 p.hook(b+0x01,LdDFromH(b+0x02),length=1)                       # ld d,h
 p.hook(b+0x02,LdEFromL(b+0x03),length=1)                       # ld e,l
 p.hook(b+0x03,LdHFromB(b+0x04),length=1)                       # ld h,b
 p.hook(b+0x04,LdLFromC(b+0x05),length=1)                       # ld l,c
 p.hook(b+0x05,PlaceStringSite(b+0x08),length=3)                 # call PlaceString (proved boundary)
 p.hook(b+0x08,LdHFromD(b+0x09),length=1)                        # ld h,d
 p.hook(b+0x09,LdLFromE(b+0x0a),length=1)                        # ld l,e
 p.hook(b+0x0a,IncHL(b+0x0b),length=1)                          # inc hl
 p.hook(b+0x0b,Jmp(CONT),length=2)                              # jr NextTextCommand
 st=p.factory.blank_state(addr=b);set_assembly_registers(st,v);setup(st,v,False)
 print('ENTRY regs:',{r:hex(st.solver.eval(getattr(st.regs,r))) for r in ('a','f','b','c','d','e','h','l')},flush=True)
 sp=STACK-2
 ...
 print(f'ASM exploration done: found={len(m.found)} errored={len(m.errored)}',flush=True)
 assert not m.errored and len(m.found)==1
 out=[]
 for x in m.found:
   ar=assembly_registers(x)
   out.append(E(**{**{k:v for k,v in ar.items() if k not in ('h','l')},'hl':claripy.Concat(x.regs.h,x.regs.l)},window=claripy.Concat(*(x.memory.load(DEST+i,1) for i in range(STRLEN+2))),string=claripy.Concat(*(x.memory.load(TEXTPTR+i,1) for i in range(len(STRING)))),constraints=tuple(x.solver.constraints)))
 return out
def native(v):
 p=angr.Project(ELF,auto_load_libs=False)
 f=p.loader.find_symbol('port_text_command_start');assert f
 s=p.factory.call_state(f.rebased_addr,NS,NM);store_native_registers(s,NS,v);setup(s,v,True)
 print('native run starting',flush=True)
 m=p.factory.simulation_manager(s);m.run()
 print(f'native run done: deadended={len(m.deadended)}',flush=True)
 assert not m.errored and len(m.deadended)==1
 out=[]
 for x in m.deadended:
   nr=native_registers(x,NS)
   out.append(E(**{**{k:v for k,v in nr.items() if k not in ('h','l')},'hl':claripy.Concat(nr['h'],nr['l'])},window=claripy.Concat(*(x.memory.load(NM+DEST+i,1) for i in range(STRLEN+2))),string=claripy.Concat(*(x.memory.load(NM+TEXTPTR+i,1) for i in range(len(STRING)))),constraints=tuple(x.solver.constraints)))
 return out
@pytest.mark.skipif(not ELF.exists() or not ROM.exists() or not SYMS.exists(),reason='build')
def test_text_command_start_pathwise_equivalence():
 v=inputs('tsta');assert_pathwise_equivalent(assembly(v),native(v),('a','f','b','c','d','e','hl','window','string'))
