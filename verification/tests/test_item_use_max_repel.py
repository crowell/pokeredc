...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "ItemUseMaxRepel")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_item_use_max_repel")
...
def test_item_use_max_repel_symbolic_equivalence() -> None:
    inputs = symbolic_registers("iumr")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_item_use_max_repel_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "ItemUseMaxRepel")
    assert linked_bytes(ROM, loc, 5) == bytes.fromhex("06fac30560")
