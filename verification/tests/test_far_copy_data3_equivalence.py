...
FIELDS=('requested_bank','loaded_bank','rom_bank','copy_b','copy_c')
...
class Endpoint:
 ...
 requested_bank:claripy.ast.BV;loaded_bank:claripy.ast.BV;rom_bank:claripy.ast.BV;copy_b:claripy.ast.BV;copy_c:claripy.ast.BV;constraints:tuple[claripy.ast.Bool,...]
...
class CopyDataSummary(angr.SimProcedure):
 def run(self)->None:self.state.regs.b=self.state.globals['copy_b'];self.state.regs.c=self.state.globals['copy_c'];self.jump(self.state.addr+3)
...
class RestoreBank(angr.SimProcedure):
 def run(self)->None:self.state.globals['loaded_bank']=self.state.globals['loaded_bank_original'];self.state.globals['rom_bank']=self.state.globals['loaded_bank_original'];self.jump(self.state.addr+6)
class Boundary(angr.SimProcedure):
 def run(self)->None:self.jump(DONE)
def _assembly(i):
 l=symbol_location(SYMBOLS,'FarCopyData3');q=l.address;p=angr.Project(rom_window(ROM,l.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={'backend':'blob','arch':ArchPcode('z80:LE:16:default'),'base_addr':0,'entry_point':q})
 p.hook(q,StoreRequested(),length=2);p.hook(q+2,LoadField('loaded_bank',q+4,2),length=2);p.hook(q+4,PushNoOp(),length=1);p.hook(q+5,LoadField('requested_bank',q+7,2),length=2);p.hook(q+7,SetRomBank(),length=5)
 for off in (12,13,14,15,16,17,21,22):p.hook(q+off,PushNoOp(),length=1)
 p.hook(q+18,CopyDataSummary(),length=3);p.hook(q+23,PopAF(),length=1);p.hook(q+24,RestoreBank(),length=6);p.hook(q+30,Boundary(),length=1)
...
def _native(i):
 p=angr.Project(NATIVE_ELF,auto_load_libs=False);fn=p.loader.find_symbol('port_far_copy_data3');assert fn;s=p.factory.call_state(fn.rebased_addr,NATIVE_STATE);store_native_registers(s,NATIVE_STATE,i)
...
def test_far_copy_data3_pathwise_equivalence():
 i=symbolic_registers('fcd3');i['requested_bank']=i['a'];i['loaded_bank']=claripy.BVS('fcd3_loaded_bank',8);i['rom_bank']=claripy.BVS('fcd3_rom_bank',8);i['copy_b']=claripy.BVS('fcd3_copy_b',8);i['copy_c']=claripy.BVS('fcd3_copy_c',8)
 assert_pathwise_equivalent(_assembly(i),_native(i),('a','f','b','c','d','e','h','l',*FIELDS))
def test_far_copy_data3_exact_body():
 l=symbol_location(SYMBOLS,'FarCopyData3');assert linked_bytes(ROM,l,30)==bytes.fromhex('e08bf0b8f5f08be0b8ea0020e5d5d5545de1cdb500d1e1f1e0b8ea0020c9')
