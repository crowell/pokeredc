...
class PaletteSummary(angr.SimProcedure):
    def run(self) -> None:
        self.state.regs.a = claripy.BVV(0xFE, 8)
        self.state.regs.b = claripy.BVV(0xFE, 8)
        self.state.regs.c = claripy.BVV(0xF8, 8)
        self.state.regs.f = claripy.BVV(0, 8)
        self.state.memory.store(R_BGP, claripy.BVV(0xFE, 8))
        self.jump(DONE)


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "AnimationUnusedPalette1")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_animation_unused_palette1_player")
...
def test_animation_unused_palette1_player_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_unused_palette1")
    values["palette"] = claripy.BVS("animation_unused_palette1_value", 8)
