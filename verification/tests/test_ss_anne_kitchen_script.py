...
EXPECTED=linked_bytes(ROM,symbol_location(SYMS,'SSAnneKitchen_Script'),4)
...
def assembly(v):
 l=symbol_location(SYMS,'SSAnneKitchen_Script');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_ss_anne_kitchen_script');t=p.loader.find_symbol('port_enable_auto_text_box_drawing');assert f and t
...
def test_ss_anne_kitchen_script_pathwise_equivalence():
 v=inputs('ss_anne_kitchen_script');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'control','wait','at'))
