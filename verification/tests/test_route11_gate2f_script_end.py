...
def assembly(v):
 l=symbol_location(SYMS,'Route11Gate2FScriptEnd');t=symbol_location(SYMS,'TextScriptEnd');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_route11_gate2f_script_end');t=p.loader.find_symbol('port_text_script_end');assert f and t
...
def test_route11_gate2f_script_end_pathwise_equivalence():
 v=inputs('route11_gate2f_script_end');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'tse'))
