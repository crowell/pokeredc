...
class Endpoint:
    ...
    player_high: claripy.ast.BV
    player_low: claripy.ast.BV
    enemy_high: claripy.ast.BV
    enemy_low: claripy.ast.BV
    constraints: tuple[claripy.ast.Bool, ...]


def _addresses(symbol: str) -> dict[str, int]:
    stat_name = "Speed" if symbol == "QuarterSpeedDueToParalysis" else "Attack"
    player_stat = symbol_location(SYMBOLS, f"wBattleMon{stat_name}").address
    enemy_stat = symbol_location(SYMBOLS, f"wEnemyMon{stat_name}").address
    return {
        ...
        "player_high": player_stat,
        "player_low": player_stat + 1,
        "enemy_high": enemy_stat,
        "enemy_low": enemy_stat + 1,
    }
...
def _assembly_endpoints(
    ...
    project.hook(
        location.address,
        Sm83LoadAHighImmediate(0xF3, location.address + 2),
        length=2,
    )
    ...
    for offset, name in status_loads:
        project.hook(
            ...
            Sm83LoadAImmediate(addresses[name], location.address + offset + 3),
            length=3,
        )
    for offset in decrements:
        project.hook(
            location.address + offset,
            Sm83LoadAAtHlDecrement(location.address + offset + 1),
            length=1,
        )
    for offset in shifts:
        project.hook(
            location.address + offset,
            Sm83SrlRegister("a", location.address + offset + 2),
            length=2,
        )
    for offset in rotates:
        project.hook(
            location.address + offset,
            Sm83RrRegister("b", location.address + offset + 2),
            length=2,
        )
    for offset in stores:
        project.hook(
            location.address + offset,
            Sm83StoreAAtHlIncrement(location.address + offset + 1),
            length=1,
        )
...
@pytest.mark.parametrize(
    ...
    [
        ("QuarterSpeedDueToParalysis", "port_quarter_speed_due_to_paralysis"),
        ("HalveAttackDueToBurn", "port_halve_attack_due_to_burn"),
    ],
)
def test_status_penalty_symbolic_equivalence(
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            61,
            "f0f3a7281cfa18d0e640c8212ad03a477ecb3fcb18cb3fcb1822b02002060170c9fae9cfe640c821fbcf3a477ecb3fcb18cb3fcb1822b02002060170c9",
        ),
        (
            "HalveAttackDueToBurn",
            53,
            "f0f3a72818fa18d0e610c82126d03a477ecb3fcb1822b02002060170c9fae9cfe610c821f7cf3a477ecb3fcb1822b02002060170c9",
        ),
    ],
)
def test_status_penalty_machine_code_is_accounted_for(
