...
class CalcChecksumBegin(angr.SimProcedure):
    """Model just the first instruction of CalcCheckSum: ld d, 0"""
...
def assembly(i: dict) -> list:
    ...
    # Hook just the first 2 bytes: ld d, 0 (16 00)
    p.hook(q, CalcChecksumBegin(DONE), length=2)
...
def native(i: dict) -> list:
    ...
    fn = p.loader.find_symbol("port_calc_checksum_begin")
    assert fn
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "CalcCheckSum")
    # First 2 bytes: ld d, 0 (16 00)
    assert linked_bytes(ROM, loc, 2) == bytes.fromhex("1600")