...
# BattleAnimCopyTileMapToVRAM: 7c e0bd 7d e0bc c3d73d
#   ld a,h / ldh [dest+1],a / ld a,l / ldh [dest],a / jp Delay3
...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    # The two `ld a, r` register loads (opcodes 7C/7D) clear all flags on
    # SM83; the Z80 pcode backend leaves C set there, so shim them. The two
    # `ldh [a8], a` stores (opcode E0) are absent from the Z80.
    # Bytes: 7c e0bd 7d e0bc c3d73d
    project.hook(base + 0x00, Sm83LoadAFromRegister("h", base + 0x01), length=1)
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol(
        "port_battle_anim_copy_tilemap_to_vram"
    )
...
def test_battle_anim_copy_tilemap_to_vram_symbolic_equivalence() -> None:
    i = _inputs("bat")
