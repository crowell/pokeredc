...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "TransformEffect")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_transform_effect")
...
def test_transform_effect_symbolic_equivalence() -> None:
    inputs = symbolic_registers("te")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_transform_effect_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "TransformEffect")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("21b17a060ec3d635")
