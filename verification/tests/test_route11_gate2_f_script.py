...
def assembly(v):
 l=symbol_location(SYMS,'Route11Gate2F_Script');t=symbol_location(SYMS,'DisableAutoTextBoxDrawing');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_route11_gate2_f_script');t=p.loader.find_symbol('port_disable_auto_text_box_drawing');assert f and t
...
def test_route11_gate2_f_script_pathwise_equivalence():
 v=inputs('route11_gate2_f_script');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'control','wait','call'))
