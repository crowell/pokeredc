...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "SelectMenuItem_CursorUp")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_select_menu_item_cursor_up")
...
def test_select_menu_item_cursor_up_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("select_menu_item_cursor_up")
    values["current_menu_item"] = claripy.BVS("select_menu_item_cursor_up_item", 8)
