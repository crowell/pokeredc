...
DEST_BASE = 0xCC00  # concrete destination (wBuffer=0xcee9 is above this range)


@lru_cache(maxsize=None)
def _pp_inputs() -> tuple[claripy.ast.BV, ...]:
    # Symbolic source bytes shared between the asm and native endpoints.
    return tuple(claripy.BVS(f"fcd_src{i}", 8) for i in range(FAR_COPY_LEN))
...
def _assembly_endpoint(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    # ld [wBuffer], a (0xEA); ldh a,[hLoadedROMBank] (0xF0); ld a,[wBuffer]
    # (0xFA); ldh [hLoadedROMBank], a (0xE0); ld [rROMB], a (0xEA) — the bank
    # switch is a no-op for the observable in the flat memory model.
    project.hook(base + 0x00, Sm83StoreAImmediate(0xCEE9, base + 0x03), length=3)
    project.hook(base + 0x03, Sm83LoadAHighImmediate(0xB8, base + 0x05), length=2)
    project.hook(base + 0x06, Sm83LoadAImmediate(0xCEE9, base + 0x09), length=3)
    project.hook(base + 0x09, Sm83StoreAHighImmediate(0xB8, base + 0x0B), length=2)
    project.hook(base + 0x0B, Sm83StoreAImmediate(0x2000, base + 0x0E), length=3)
    ...
    project.hook(base + 0x12, Sm83StoreAHighImmediate(0xB8, base + 0x14), length=2)
    project.hook(base + 0x14, Sm83StoreAImmediate(0x2000, base + 0x17), length=3)
...
def test_far_copy_data_symbolic_equivalence() -> None:
    inputs = symbolic_registers("fcd")
...
def test_far_copy_data_exact_linked_body() -> None:
    location = symbol_location(SYMBOLS, "FarCopyData")
    expected = bytes.fromhex("eae9cef0b8f5fae9cee0b8ea0020cdb500f1e0b8ea0020c9")
