...
PRINT_CURSOR = 0xC4B9
EXPECTED = bytes.fromhex("21ba2acd493cfa32d7cbafea32d7c3df29")
...
class HoldTextBoundary(angr.SimProcedure):
    def run(self, register_address=None, memory_address=None) -> None:  # type: ignore[override]
        if self.state.arch.name.startswith("AMD64"):
            pointer = self.state.regs.rdi
            memory = self.state.regs.rsi
            self.state.globals["hold_call"] = claripy.Concat(*(
                self.state.memory.load(pointer + offset, 1)
                for offset in range(8)))
            self.state.globals["hold_status"] = self.state.memory.load(
                memory + W_STATUS_FLAGS6, 1)
...
def _setup(state: angr.SimState, base: int,
    ...
    state.memory.store(base + W_STATUS_FLAGS6, values["status"])
    ...
    return Endpoint(
        **(native_registers(state, NATIVE_STATE)
        ...
        memory=claripy.Concat(
            ...
            state.memory.load(base + W_STATUS_FLAGS6, 1),
        ),
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "DisplayPlayerBlackedOutText")
    ...
    project.hook(location.address + 6,
                 Sm83LoadAImmediate(W_STATUS_FLAGS6, location.address + 9),
                 length=3)
    project.hook(location.address + 0x0b,
                 Sm83StoreAImmediate(W_STATUS_FLAGS6, location.address + 0x0e),
                 length=3)
    project.hook(hold.address, HoldTextBoundary(), length=1)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_display_player_blacked_out_text")
    print_text = project.loader.find_symbol("port_print_text")
    hold = project.loader.find_symbol("port_hold_text_display_open")
    assert function is not None and print_text is not None and hold is not None
    project.hook(print_text.rebased_addr, PrintTextBoundary())
    project.hook(hold.rebased_addr, HoldTextBoundary())
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(),
                    reason="run `make red`")
def test_display_player_blacked_out_text_pathwise_equivalence() -> None:
    values = symbolic_registers("display_player_blacked_out_text")
    values["textbox"] = claripy.BVS("display_player_blacked_out_textbox", 8)
    values["status"] = claripy.BVS("display_player_blacked_out_status", 8)
    for name in REGISTERS:
        values[f"out_{name}"] = (
            claripy.Concat(claripy.BVS("display_player_blacked_out_flags", 4),
                           claripy.BVV(0, 4))
            if name == "f" else
            claripy.BVS(f"display_player_blacked_out_out_{name}", 8)
        )
    assert_pathwise_equivalent(
        _assembly(values), _native(values),
        (*REGISTERS, "memory", "print_call", "hold_call"),
    )
