...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "LeechSeedEffect")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_leech_seed_effect")
...
def test_leech_seed_effect_symbolic_equivalence() -> None:
    inputs = symbolic_registers("lse")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_leech_seed_effect_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "LeechSeedEffect")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("21a97e060ac3d635")
