...
PORTS = (
    ("PrintPlayerMon1Text", "port_print_player_mon1_text"),
    ("PrintComeBackText", "port_print_come_back_text"),
    ("LoadPresentsGraphic", "port_load_presents_graphic"),
)
...
def test_text_pointer_load_symbolic_equivalence(
...
@pytest.mark.parametrize(
    ("symbol", "packet_hl"),
    [
        ("PrintPlayerMon1Text", "PlayerMon1Text"),
        ("PrintComeBackText", "ComeBackText"),
        ("LoadPresentsGraphic", None),
    ],
)
def test_text_pointer_load_exact_linked_body(
    symbol: str, packet_hl: str | None
) -> None:
    location = symbol_location(SYMBOLS, symbol)
    if packet_hl is None:
        # LoadPresentsGraphic is a single RET.
        assert linked_bytes(ROM, location, 1) == bytes((0xC9,))
        return
    hl_addr = symbol_location(SYMBOLS, packet_hl).address
    # ld hl, <hl>; ret
    expected = bytes((0x21, hl_addr & 0xFF, hl_addr >> 8, 0xC9))
    assert linked_bytes(ROM, location, 4) == expected
