...
def assembly(v):
 l=symbol_location(SYMS,'RockTunnelPokecenter_Script');t=symbol_location(SYMS,'Serial_TryEstablishingExternallyClockedConnection');a=symbol_location(SYMS,'EnableAutoTextBoxDrawing')
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_rock_tunnel_pokecenter_script');se=p.loader.find_symbol('port_serial_try_establishing_externally_clocked_connection');at=p.loader.find_symbol('port_enable_auto_text_box_drawing');assert f and se and at
...
def test_rock_tunnel_pokecenter_script_pathwise_equivalence():
 v=inputs('rock_tunnel_pokecenter_script');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'sb','rx','sc','control','wait','se','at'))
