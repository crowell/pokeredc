...
class LoopOnce(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        if self.state.globals.get("copy_entered", False):
            self.jump(LOOP_BOUNDARY)
            return
        self.state.globals["copy_entered"] = True
        self.state.regs.a = self.state.globals["fetched"]
        self.jump(self._next_address)
...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "CopyString")
    ...
    project.hook(location.address, LoopOnce(location.address + 1), length=1)
    project.hook(location.address + 2, StoreIncrement(location.address + 3), length=1)
    project.hook(location.address + 3, Sm83CpImmediate(0x50, location.address + 5), length=2)
    ...
    return [
        Endpoint(
            **assembly_registers(end), written=end.globals["written"],
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_copy_string_step")
    assert function is not None
    state = project.factory.call_state(function.rebased_addr, NATIVE_STATE, claripy.ZeroExt(56, inputs["fetched"]))
    store_native_registers(state, NATIVE_STATE, inputs)
    state.memory.store(NATIVE_STATE + 8, inputs["written"])
    manager = project.factory.simulation_manager(state)
    manager.run()
    assert not manager.errored
    return [
        Endpoint(
            **native_registers(end, NATIVE_STATE), written=end.memory.load(NATIVE_STATE + 8, 1),
...
def test_copy_string_one_step_inductive_equivalence() -> None:
    inputs = symbolic_registers("copy_string_step")
    inputs["fetched"] = claripy.BVS("copy_string_fetched", 8)
    inputs["written"] = claripy.BVS("copy_string_written", 8)
    assert_pathwise_equivalent(_assembly(inputs), _native(inputs), (*REGISTERS, "written", "continuation"))


@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_copy_string_exact_linked_body() -> None:
    location = symbol_location(SYMBOLS, "CopyString")
    assert linked_bytes(ROM, location, 8) == bytes.fromhex("1a1322fe5020f9c9")
