...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    state.memory.store(CRITICAL, values["critical_hit_or_ohko"])
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    state.memory.store(NATIVE_STATE + 8, values["critical_hit_or_ohko"])
    manager = project.factory.simulation_manager(state)
    manager.run()
    assert not manager.errored
    return [
...
def test_print_critical_ohko_text_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("print_critical_ohko_text")
    values["critical_hit_or_ohko"] = claripy.BVS("print_critical_ohko_text_value", 8)
