...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "HandlePoisonBurnLeechSeed_IncreaseEnemyHP")
    ...
    project.hook(base + 4, Sm83LoadAHighImmediate(0xF3, base + 6), length=2)
    project.hook(base + 6, AndA(), length=1)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_increase_enemy_hp_setup")
...
def test_increase_enemy_hp_setup_pathwise_equivalence() -> None:
    values = symbolic_registers("increase_enemy_hp")
    values["whose_turn"] = claripy.BVS("increase_enemy_hp_whose_turn", 8)
