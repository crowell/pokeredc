...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.f |= claripy.If(
            (value & 0x0F).ULT(LINK_STATE_BATTLING),
            claripy.BVV(0x10, 8),
            claripy.BVV(0, 8),
        )
        self.state.regs.f |= claripy.If(
            value.ULT(LINK_STATE_BATTLING),
            claripy.BVV(0x01, 8),
            claripy.BVV(0, 8),
        )
        self.state.regs.f |= claripy.If(
            value == LINK_STATE_BATTLING,
            claripy.BVV(0x40, 8),
            claripy.BVV(0, 8),
        )
        self.jump(DONE)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "GetEnemyMonStat")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_get_enemy_mon_stat")
...
def test_get_enemy_mon_stat_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("get_enemy_mon_stat")
    values["link_state"] = claripy.BVS("get_enemy_mon_stat_link_state", 8)
