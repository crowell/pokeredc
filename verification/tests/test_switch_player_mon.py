...
class SetupHL(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.l = claripy.BVV(0xD1, 8)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "SwitchPlayerMon")
    ...
    project.hook(base + 3, Boundary(), length=2)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_switch_player_mon")
...
def test_switch_player_mon_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("switch_player_mon")
