...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "GetTrainerName")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_get_trainer_name")
...
def test_get_trainer_name_symbolic_equivalence() -> None:
    inputs = symbolic_registers("gtn")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_get_trainer_name_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "GetTrainerName")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("060421587ac3d635")
