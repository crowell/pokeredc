...
class CalcDifference(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        b = self.state.regs.b
        sub_result = a - b
        sub_f = sm83_flags_to_z80(sm83_sub(a, b))
        borrow = a < b
        # Path A (no borrow): result and subtraction flags stand.
        # Path B (borrow): a = ~result + 1, f = C only.
        abs_result = ~sub_result + 1
        self.state.regs.a = claripy.If(borrow, abs_result, sub_result)
        self.state.regs.f = claripy.If(borrow, claripy.BVV(0x01, 8), sub_f)
        self.jump(self._n)


def inputs() -> dict:
    return symbolic_registers("cd")


def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "CalcDifference")
    # sub b / ret nc / cpl / add a,1 / scf / ret  (7 bytes, bank 0).
    q = loc.address
    p = angr.Project(
    ...
    p.hook(q, CalcDifference(DONE), length=7)
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_calc_difference")
...
def test_transition_equivalence() -> None:
    i = inputs()
    assert_pathwise_equivalent(assembly(i), native(i), ("a", "f", "b"))


@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "CalcDifference")
    assert linked_bytes(ROM, loc, 7) == bytes.fromhex("90d02fc60137c9")
