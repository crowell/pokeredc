...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "GetDefaultName.foundName")
    base = loc.address
    ...
    project.hook(base, CopyInputRegister("h", "input_d", base + 1), length=1)
    project.hook(base + 1, CopyInputRegister("l", "input_e", base + 2), length=1)
    project.hook(base + 2, LoadPairImmediate("de", 0xcd6d, base + 5), length=3)
    project.hook(base + 5, LoadPairImmediate("bc", 0x0014, base + 8), length=3)
    project.hook(base + 8, Boundary(), length=3)
    state = project.factory.blank_state(addr=base)
    set_assembly_registers(state, inputs)
    state.globals["input_d"] = inputs["d"]
    state.globals["input_e"] = inputs["e"]
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_get_default_name_found_name")
...
def test_get_default_name_found_name_symbolic_equivalence() -> None:
    inputs = symbolic_registers("gdnfn")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_get_default_name_found_name_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "GetDefaultName.foundName")
    assert linked_bytes(ROM, loc, 11) == bytes.fromhex("626b116dcd011400c3b500")
