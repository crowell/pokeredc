...
def _assembly(values: dict[str, claripy.ast.BV]):
    location = symbol_location(SYMBOLS, "PlacePKMN")
    target = symbol_location(SYMBOLS, "PlaceCommandCharacter")
    expected = bytes.fromhex("d511791a181c")
...
def _native(values: dict[str, claripy.ast.BV]):
    ...
    function = project.loader.find_symbol("port_place_pkmn")
...
def test_place_pkmn_pathwise_equivalence() -> None:
    values = symbolic_registers("place_pkmn")
