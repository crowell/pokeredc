...
class WakeUpEntirePartyBegin(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        # WakeUpEntirePartyBegin (beginning of WakeUpEntireParty):
        #   ld de, PARTYMON_STRUCT_LENGTH
        #   ld c, PARTY_LENGTH
        self.state.regs.d = claripy.BVV(0x00, 8)
        self.state.regs.e = claripy.BVV(0x2C, 8)
        self.state.regs.c = claripy.BVV(0x06, 8)
        self.jump(self._n)
...
def assembly(i: dict) -> list:
    loc = symbol_location(SYMBOLS, "WakeUpEntireParty")
    q = loc.address
    p = angr.Project(
    ...
    p.hook(q, WakeUpEntirePartyBegin(DONE), length=5)
...
def native(i: dict) -> list:
    ...
    fn = p.loader.find_symbol("port_wake_up_entire_party_begin")
    assert fn
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "WakeUpEntireParty")
    assert linked_bytes(ROM, loc, 5) == bytes.fromhex("112c000e06")