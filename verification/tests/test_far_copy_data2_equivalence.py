...
EXPECTED_BODY = bytes.fromhex("e08bf0b8f5f08be0b8ea0020cdb500f1e0b8ea0020c9")
...
class NativeCopyDataSummary(angr.SimProcedure):
    ...
    def run(
        ...
        self.state.memory.store(
            memory + MARKER, self.state.globals["copy_marker"]
        )
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    project.hook(base + 12, CopyDataSummary(base + 15), length=3)
    project.hook(base + 16, StoreField("loaded_bank", base + 18), length=2)
    project.hook(base + 18, StoreField("rom_bank", base + 21), length=3)
    ...
    return [
        Endpoint(
            ...
            **{field: end.globals[field] for field in FIELDS},
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    return [
        Endpoint(
            ...
            **{
                field: end.memory.load(NATIVE_STATE + offset, 1)
                for offset, field in enumerate(FIELDS, 8)
            },
...
def test_far_copy_data2_pathwise_equivalence() -> None:
    values = _inputs("far_copy_data2")
    assert_pathwise_equivalent(
        _assembly(values),
        _native(values),
        (*REGISTERS, *FIELDS, "marker"),
    )
