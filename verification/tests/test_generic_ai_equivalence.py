...
class GenericAI(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        # and a: a unchanged; f = H, plus Z when a == 0.
        a = self.state.regs.a
        self.state.regs.a = a
...
def inputs() -> dict:
    return symbolic_registers("ga")


def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "GenericAI")
    # and a / ret  (2 bytes, bank 14).
    q = loc.address
    p = angr.Project(
    ...
    p.hook(q, GenericAI(DONE), length=2)
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_generic_ai")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "GenericAI")
    assert linked_bytes(ROM, loc, 2) == bytes.fromhex("a7c9")
