...
EXPECTED=bytes.fromhex('fe20d0c3f267')
...
def assembly(v):
 l=symbol_location(SYMS,'BlackbeltAI');t=symbol_location(SYMS,'AIUseXAttack');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
 ...
 p.hook(b+0,Sm83CpImmediate(0x20,b+2),length=2)
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_blackbelt_ai');t=p.loader.find_symbol('port_ai_use_x_attack');assert f and t
...
def test_blackbelt_ai_pathwise_equivalence():
 v=inputs('blackbelt_ai');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ai'))
