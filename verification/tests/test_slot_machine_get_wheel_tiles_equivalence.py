...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "SlotMachine_GetWheelTiles")
    q = symbol_location(SYMBOLS, "SlotMachine_GetWheelTiles.loop").address
    p = angr.Project(
    ...
    p.hook(q + 3, IncHl(q + 4), length=1)   # inc hl
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_slot_machine_get_wheel_tiles_step")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "SlotMachine_GetWheelTiles")
    assert linked_bytes(ROM, loc, 14) == bytes.fromhex("4f0600090e032a1213230d20f9c9")
