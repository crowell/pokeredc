...
def _assembly(inputs:dict[str,claripy.ast.BV])->list[Endpoint]:
 loc=symbol_location(SYMBOLS,"EffectCallBattleCore"); base=loc.address; project=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":base}); project.hook(base+2,Boundary(),length=3)
...
def _native(inputs:dict[str,claripy.ast.BV])->list[Endpoint]:
 project=angr.Project(NATIVE_ELF,auto_load_libs=False); function=project.loader.find_symbol("port_effect_call_battle_core"); assert function is not None; state=project.factory.call_state(function.rebased_addr,NATIVE_STATE,claripy.BVV(0,64)); store_native_registers(state,NATIVE_STATE,inputs); manager=project.factory.simulation_manager(state); manager.run(); assert not manager.errored and len(manager.deadended)==1; end=manager.deadended[0]; return [Endpoint(**native_registers(end,NATIVE_STATE),constraints=tuple(end.solver.constraints))]
@pytest.mark.skipif(not NATIVE_ELF.exists(),reason="run `make -C verification native`")
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(),reason="run `make red`")
def test_effect_call_battle_core_symbolic_equivalence()->None:
 inputs=symbolic_registers("ecbc"); assert_pathwise_equivalent(_assembly(inputs),_native(inputs),("a","f","b","c","d","e","h","l"))
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(),reason="run `make red`")
def test_effect_call_battle_core_exact_linked_body()->None:
 loc=symbol_location(SYMBOLS,"EffectCallBattleCore"); assert linked_bytes(ROM,loc,5)==bytes.fromhex("060fc3d635")
