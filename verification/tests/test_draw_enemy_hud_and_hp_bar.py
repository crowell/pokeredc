...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "DrawEnemyHUDAndHPBar")
    ...
    project.hook(base + 3, Setup(), length=6)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_draw_enemy_hud_and_hp_bar")
...
def test_draw_enemy_hud_and_hp_bar_setup_pathwise_equivalence() -> None:
    values = symbolic_registers("draw_enemy_hud_and_hp_bar")
    values["auto_bg"] = claripy.BVS("draw_enemy_hud_and_hp_bar_auto_bg", 8)
