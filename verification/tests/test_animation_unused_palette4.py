...
EXPECTED_ENTRY = bytes.fromhex("014040")


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    entry = symbol_location(SYMBOLS, "AnimationUnusedPalette4")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol(
        "port_animation_unused_palette4_player"
    )
...
def test_animation_unused_palette4_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_unused_palette4")
    values["on_sgb"] = claripy.BVS("animation_unused_palette4_on_sgb", 8)
    values["palette"] = claripy.BVS("animation_unused_palette4_palette", 8)
