...
class PaletteSummary(angr.SimProcedure):
    def run(self) -> None:
        for register in ("a", "b", "c"):
            setattr(self.state.regs, register, claripy.BVV(0x40, 8))
        self.state.regs.f = claripy.BVV(0, 8)
        self.state.memory.store(R_BGP, claripy.BVV(0x40, 8))
        self.jump(DONE)


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "AnimationUnusedPalette4")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_animation_unused_palette4_player")
...
def test_animation_unused_palette4_player_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_unused_palette4")
    values["palette"] = claripy.BVS("animation_unused_palette4_value", 8)
