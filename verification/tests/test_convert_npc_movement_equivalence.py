...
class ConvertNPCMovementBegin(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        # asm: push hl; ld b,a; ld hl,0x79d2; (jumps into loop)
        # We model just the begin portion: b=a, h=0x79, l=0xd2
        a = self.state.regs.a
        self.state.regs.b = a
        self.state.regs.h = claripy.BVV(0x79, 8)
        self.state.regs.l = claripy.BVV(0xD2, 8)
        # push hl is internal (preserves old hl for later pop); not observable here
        self.jump(self._n)
...
def assembly(i: dict) -> list[E]:
    ...
    # Hook first 6 bytes: push hl; ld b,a; ld hl,0x79d2
    p.hook(q, ConvertNPCMovementBegin(DONE), length=6)
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_convert_npc_movement_direction_begin")
    assert fn
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "ConvertNPCMovementDirectionToJoypadMask")
    # First 6 bytes: e5 47 21 d2 79 2a
    assert linked_bytes(ROM, loc, 6) == bytes.fromhex("e54721d2792a")