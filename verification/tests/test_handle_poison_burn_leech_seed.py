...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "HandlePoisonBurnLeechSeed")
    ...
    project.hook(base + 9, Boundary(), length=2)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_handle_poison_burn_leech_seed")
...
def test_handle_poison_burn_leech_seed_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("handle_poison_burn_leech_seed")
    values["whose_turn"] = claripy.BVS("handle_poison_burn_leech_seed_whose_turn", 8)
