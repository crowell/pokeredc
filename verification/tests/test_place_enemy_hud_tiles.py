...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.l = claripy.BVV(0xC9, 8)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "PlaceEnemyHUDTiles")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_place_enemy_hud_tiles")
...
def test_place_enemy_hud_tiles_pathwise_equivalence() -> None:
    values = symbolic_registers("place_enemy_hud_tiles")
