...
def assembly(v):
 l=symbol_location(SYMS,'UnusableItem');t=symbol_location(SYMS,'ItemUseNotTime');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_unusable_item');t=p.loader.find_symbol('port_item_use_not_time');assert f and t
...
def test_unusable_item_pathwise_equivalence():
 v=inputs('unusable_item');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'tse'))
