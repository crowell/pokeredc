...
EXPECTED_BODY=bytes.fromhex('0e023edecda1232106c011a271cd605b111d72cd605b11b572')
...
class Summary(angr.SimProcedure):
 def run(self)->None:
  st=self.state.copy();st.regs.c=claripy.BVV(2,8);st.regs.a=claripy.BVV(0xde,8);st.regs.h=claripy.BVV(0xc0,8);st.regs.l=claripy.BVV(0x0c,8);st.globals['channel1_pointer']=claripy.BVV(0x71a2,16);st.globals['channel2_pointer']=claripy.BVV(0x721d,16);st.globals['channel3_pointer']=claripy.BVV(0x72b5,16);st.globals['sound_id']=claripy.BVV(0xde,8);st.globals['audio_rom_bank']=claripy.BVV(2,8);st.globals['audio_saved_bank']=claripy.BVV(2,8);st.globals['fade_out_control']=claripy.BVV(0,8);st.globals['play_music_called']=claripy.BVV(1,8);self.inhibit_autoret=True;self.successors.add_successor(st,DONE,claripy.BoolV(True),'Ijk_Boring')
def _assembly(i):
 l=symbol_location(SYMBOLS,'Music_RivalAlternateStart');q=l.address;p=angr.Project(rom_window(ROM,l.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={'backend':'blob','arch':ArchPcode('z80:LE:16:default'),'base_addr':0,'entry_point':q});p.hook(q,Summary(),length=1);s=p.factory.blank_state(addr=q);set_assembly_registers(s,i)
...
def _native(i):
 p=angr.Project(NATIVE_ELF,auto_load_libs=False);fn=p.loader.find_symbol('port_music_rival_alternate_start');assert fn;s=p.factory.call_state(fn.rebased_addr,NATIVE_STATE);store_native_registers(s,NATIVE_STATE,i)
...
def test_music_rival_alternate_start_pathwise_equivalence():
 i=symbolic_registers('rival_start')
 for f in FIELDS:i[f]=claripy.BVS('rival_start_'+f,16 if 'pointer' in f else 8)
 assert_pathwise_equivalent(_assembly(i),_native(i),('a','f','b','c','d','e','h','l',*FIELDS))
def test_music_rival_alternate_start_exact_linked_body():
 l=symbol_location(SYMBOLS,'Music_RivalAlternateStart');assert linked_bytes(ROM,l,len(EXPECTED_BODY))==EXPECTED_BODY
