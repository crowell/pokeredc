...
WRAPPER_EXPECTED = bytes.fromhex("3e24c36d3e")
...
class AssemblyPointerSummary(angr.SimProcedure):
    """Complete proven GetPredefPointer transition for predef ID $24."""
    ...
    def run(self) -> None:
        ...
        registers.a = claripy.BVV(0x41, 8)
        registers.f = claripy.BVV(0, 8)
        registers.d = claripy.BVV(0x7E, 8)
        registers.e = claripy.BVV(0xE7, 8)
        registers.h = claripy.BVV(0x41, 8)
        registers.l = claripy.BVV(0x25, 8)
        self.jump(self._next_address)


class AssemblyHorizontalTarget(angr.SimProcedure):
    """Arbitrary matching transition of the completely proved horizontal predef."""
    ...
    def run(self) -> None:
        ...
        self.state.memory.store(MUTATE_WX, self.state.globals["target_out_mutate"])
        self.state.memory.store(WX, self.state.globals["target_out_wx"])
...
class NativeHorizontalTarget(angr.SimProcedure):
    def run(self) -> None:
        address = self.state.regs.rdi
        self.state.globals["target_call"] = self.state.memory.load(address, 21)
        ...
        self.state.memory.store(address + 14, self.state.globals["target_out_mutate"])
        self.state.memory.store(address + 15, self.state.globals["target_out_wx"])
...
def _setup_globals(state: angr.SimState, values: dict[str, claripy.ast.BV]) -> None:
    state.globals["target_call"] = claripy.BVV(0, 21 * 8)
    for name in REGISTERS:
        state.globals[f"target_out_{name}"] = values[f"target_out_{name}"]
    for name in ("mutate", "wx"):
        state.globals[f"target_out_{name}"] = values[f"target_out_{name}"]
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    target = symbol_location(SYMBOLS, "PredefShakeScreenHorizontally")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    target = project.loader.find_symbol(
        "port_predef_shake_screen_horizontally_private"
    )
    assert function is not None and target is not None
    project.hook(target.rebased_addr, NativeHorizontalTarget())
    state = project.factory.call_state(function.rebased_addr, NATIVE_STATE)
    store_native_registers(state, NATIVE_STATE, values)
    state.memory.store(NATIVE_STATE + 14, values["mutate"])
    state.memory.store(NATIVE_STATE + 15, values["wx"])
    state.memory.store(NATIVE_STATE + 19, values["loaded_bank"])
    state.memory.store(NATIVE_STATE + 20, values["rom_bank"])
    _setup_globals(state, values)
    ...
    return [
        Endpoint(
            **native_registers(end, NATIVE_STATE),
            state=end.memory.load(NATIVE_STATE + 8, 13),
            target_call=end.globals["target_call"],
            constraints=tuple(end.solver.constraints),
        )
...
def test_animation_shake_screen_horizontally_fast_pathwise_equivalence() -> None:
    values = _inputs("animation_shake_screen_horizontally_fast")
