...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "AnimationShakeScreenHorizontallyFast")
    ...
    project.hook(location.address, ShakeSummary(), length=1)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_animation_shake_screen_horizontally_fast")
...
def test_animation_shake_screen_horizontally_fast_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_shake_screen_horizontally_fast")
