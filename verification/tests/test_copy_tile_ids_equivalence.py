...
class AddB(angr.SimProcedure):
    """Models `add b` (a = fetched + b) and the ADD flag set (N cleared)."""
    ...
    def run(self) -> None:  # type: ignore[override]
        self.inhibit_autoret = True
        f8 = self.state.globals["fetched"]
        b8 = self.state.regs.b
        sum9 = claripy.ZeroExt(1, f8) + claripy.ZeroExt(1, b8)
        a_ast = sum9[7:0]
        self.state.regs.a = a_ast
        z = a_ast == 0
        c = sum9[8]
        low = claripy.ZeroExt(4, f8 & 0xF) + claripy.ZeroExt(4, b8 & 0xF)
        h = low[4]
        # shim-Z80 layout: Z@bit6, N@bit1=0, H@bit4, C@bit0.
        self.state.regs.f = claripy.Concat(
            claripy.BVV(0, 1), z, claripy.BVV(0, 1), h,
            claripy.BVV(0, 2), claripy.BVV(0, 1), c,
        )
        self.jump(self._n)
...
class DecC(angr.SimProcedure):
    """Models `dec c` preserving the carry from the prior ADD (tile_ids_dec)."""
...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "CopyTileIDs")
    q = symbol_location(SYMBOLS, "CopyTileIDs.columnLoop").address
    p = angr.Project(
    ...
    p.hook(q + 1, AddB(q + 2), length=1)    # add b
    p.hook(q + 2, IncDe(q + 3), length=1)   # inc de
    p.hook(q + 3, StoreIncHl(q + 4), length=1)  # ld [hli], a
    p.hook(q + 4, DecC(DONE), length=1)     # dec c
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_copy_tile_ids_byte")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "CopyTileIDs")
    assert linked_bytes(ROM, loc, 28) == bytes.fromhex(
        "e5c5e5f08b471a8013220d20f9e101140009c10520eb3e01e0bae1c9"
    )
