...
EXPECTED = bytes.fromhex(
    "fa10d7ea1ed1cd9e2fcd2638fa0fd7ea1ed1cdcf2fc9"
)
GLOBALS = (
    "index",
    "type",
...
def _set_assembly_registers_from(
    state: angr.SimState, prefix: str, count_offset: int = 0
) -> None:
    for offset, name in enumerate(REGISTERS, count_offset):
...
class LoadMemory(angr.SimProcedure):
...
class AssemblyGetMonName(angr.SimProcedure):
    def __init__(self, continuation: int) -> None:
        super().__init__()
        self._continuation = continuation

    def run(self) -> None:  # type: ignore[override]
        self.state.globals["mon_call"] = claripy.Concat(
            _assembly_register_bytes(self.state),
            self.state.globals["named"],
            self.state.globals["loaded"],
            self.state.globals["rom"],
        )
        self.state.globals["trace"] = self.state.globals["trace"] * 16 + 1
        _set_assembly_registers_from(self.state, "mon_out")
        self.state.globals["named"] = self.state.globals["mon_out_8"]
        self.state.globals["loaded"] = self.state.globals["mon_out_9"]
        self.state.globals["rom"] = self.state.globals["mon_out_10"]
        for offset in range(20):
            self.state.memory.store(
                NAME_BUFFER + offset, self.state.globals[f"mon_buffer_{offset}"]
            )
        self.jump(self._continuation)


class NativeGetMonName(angr.SimProcedure):
    def run(
        ...
        self.state.globals["mon_call"] = self.state.memory.load(address, 11)
        self.state.globals["trace"] = self.state.globals["trace"] * 16 + 1
        for offset in range(11):
            self.state.memory.store(address + offset, self.state.globals[f"mon_out_{offset}"])
        for offset in range(20):
            self.state.memory.store(
                memory + NAME_BUFFER + offset,
                self.state.globals[f"mon_buffer_{offset}"],
            )
...
class AssemblyCopyToStringBuffer(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        self.state.globals["copy_call"] = _assembly_register_bytes(self.state)
        self.state.globals["trace"] = self.state.globals["trace"] * 16 + 2
        _set_assembly_registers_from(self.state, "copy_out")
        for offset in range(20):
            self.state.memory.store(
                STRING_BUFFER + offset,
                self.state.globals[f"copy_buffer_{offset}"],
            )
        self.jump(self._continuation)
...
class NativeCopyToStringBuffer(angr.SimProcedure):
    def run(
        ...
        for offset in range(8):
            self.state.memory.store(address + offset, self.state.globals[f"copy_out_{offset}"])
        for offset in range(20):
            self.state.memory.store(
                memory + STRING_BUFFER + offset,
                self.state.globals[f"copy_buffer_{offset}"],
            )
...
class AssemblyGetItemName(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.globals["trace"] = self.state.globals["trace"] * 16 + 3
        _set_assembly_registers_from(self.state, "item_out")
...
def _inputs() -> dict[str, claripy.ast.BV]:
    values = symbolic_registers("load_fossil_names")
    for name in GLOBALS:
        values[name] = claripy.BVS(f"load_fossil_names_{name}", 8)
    saved = symbolic_registers("load_fossil_names_saved")
    for name in REGISTERS:
        values[f"saved_{name}"] = saved[name]
    values["saved_bank"] = claripy.BVS("load_fossil_names_saved_bank", 8)
    for prefix, count in (("mon_out", 11), ("copy_out", 8), ("item_out", 27)):
        for offset in range(count):
            values[f"{prefix}_{offset}"] = claripy.BVS(
                f"load_fossil_names_{prefix}_{offset}", 8
            )
        values[f"{prefix}_1"] = claripy.Concat(
            claripy.BVS(f"load_fossil_names_{prefix}_flags", 4),
            claripy.BVV(0, 4),
        )
    for prefix in (
        "initial_name",
    ...
    for name in ("fossil_mon", "fossil_item", "named_memory"):
        values[name] = claripy.BVS(f"load_fossil_names_{name}", 8)
    return values
...
def _setup(
    ...
    for prefix in ("mon_buffer", "copy_buffer", "item_buffer"):
        for offset in range(20):
            state.globals[f"{prefix}_{offset}"] = values[f"{prefix}_{offset}"]
    state.memory.store(memory_base + W_FOSSIL_MON, values["fossil_mon"])
    state.memory.store(memory_base + W_FOSSIL_ITEM, values["fossil_item"])
    ...
    for offset in range(20):
        ...
        state.memory.store(
            memory_base + STRING_BUFFER + offset,
            values[f"initial_string_{offset}"],
        )
    state.globals["mon_call"] = claripy.BVV(0, 88)
    ...
    return Endpoint(
        **registers,
        ...
        memory=claripy.Concat(
            state.memory.load(memory_base + W_FOSSIL_MON, 1),
            state.memory.load(memory_base + W_FOSSIL_ITEM, 1),
            state.memory.load(memory_base + W_NAMED_OBJECT_INDEX, 1),
            state.memory.load(memory_base + NAME_BUFFER, 20),
            state.memory.load(memory_base + STRING_BUFFER, 20),
        ),
        mon_call=state.globals["mon_call"],
...
def _assembly_project() -> tuple[angr.Project, int]:
    location = symbol_location(SYMS, "LoadFossilItemAndMonName")
    ...
    project.hook(base, LoadMemory(W_FOSSIL_MON, base + 3), length=3)
    project.hook(base + 3, StoreNamed(base + 6), length=3)
    project.hook(base + 6, AssemblyGetMonName(base + 9), length=3)
    project.hook(base + 9, AssemblyCopyToStringBuffer(base + 12), length=3)
    project.hook(base + 12, LoadMemory(W_FOSSIL_ITEM, base + 15), length=3)
    project.hook(base + 15, StoreNamed(base + 18), length=3)
    project.hook(base + 18, AssemblyGetItemName(base + 21), length=3)
    return project, base
...
def _native_project() -> tuple[angr.Project, int]:
    ...
    function = project.loader.find_symbol("port_load_fossil_item_and_mon_name")
    get_mon = project.loader.find_symbol("port_get_mon_name")
    copy = project.loader.find_symbol("port_copy_to_string_buffer")
    get_item = project.loader.find_symbol("port_get_item_name")
    assert function is not None and get_mon is not None and copy is not None and get_item is not None
    project.hook(get_mon.rebased_addr, NativeGetMonName())
...
def test_load_fossil_item_and_mon_name_pathwise_equivalence() -> None:
    values = _inputs()
