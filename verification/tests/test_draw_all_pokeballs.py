...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "DrawAllPokeballs")
    ...
    project.hook(location.address, Setup(), length=10)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_draw_all_pokeballs")
...
def test_draw_all_pokeballs_pathwise_equivalence() -> None:
    values = symbolic_registers("draw_all_pokeballs")
    values["is_in_battle"] = claripy.BVS("draw_all_pokeballs_battle", 8)
