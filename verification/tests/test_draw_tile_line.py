...
class Store(angr.SimProcedure):
 ...
 def run(self)->None:  # type: ignore[override]
  ...
  self.state.globals["entered"]=True;self.state.globals["written"]=self.state.regs.b;self.jump(self.n)
...
def assembly(i:dict[str,claripy.ast.BV])->list[E]:
 loc=symbol_location(SYMBOLS,"DrawTileLine");loop=loc.address+2;p=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":loop});p.hook(loop,Store(loop+1),length=1);p.hook(loop+1,Sm83AddHlRegisterPair("de",loop+2),length=1);p.hook(loop+2,Sm83DecRegister("c",loop+3),length=1);p.hook(loop+5,End(),length=1);s=p.factory.blank_state(addr=loop);set_assembly_registers(s,i);s.globals["written"]=i["written"];m=p.factory.simulation_manager(s);m.step();m.stashes["found"]=[]
 while m.active:
...
def native(i:dict[str,claripy.ast.BV])->list[E]:
 p=angr.Project(NATIVE_ELF,auto_load_libs=False);fn=p.loader.find_symbol("port_draw_tile_line_step");assert fn;s=p.factory.call_state(fn.rebased_addr,NATIVE_STATE);store_native_registers(s,NATIVE_STATE,i);s.memory.store(NATIVE_STATE+8,i["written"]);m=p.factory.simulation_manager(s);m.run();assert not m.errored
...
def test_step_equivalence()->None:
 i=symbolic_registers("draw_tile_line_step");i["written"]=claripy.BVS("draw_tile_line_written",8);assert_pathwise_equivalent(assembly(i),native(i),(*REGISTERS,"written","cont"))
def test_exact_body()->None:
 loc=symbol_location(SYMBOLS,"DrawTileLine");assert linked_bytes(ROM,loc,10)==bytes.fromhex("c5d570190d20fbd1c1c9")
