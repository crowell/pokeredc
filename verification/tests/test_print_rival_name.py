...
def _assembly(values: dict[str, claripy.ast.BV]):
    location = symbol_location(SYMBOLS, "PrintRivalName")
    target = symbol_location(SYMBOLS, "PlaceCommandCharacter")
    expected = bytes.fromhex("d5114ad31846")
...
def _native(values: dict[str, claripy.ast.BV]):
    ...
    function = project.loader.find_symbol("port_print_rival_name")
...
def test_print_rival_name_pathwise_equivalence() -> None:
    values = symbolic_registers("print_rival_name")
    values["h"] = claripy.BVV(DESTINATION >> 8, 8)
    values["l"] = claripy.BVV(DESTINATION & 0xFF, 8)
    assembly = _assembly(values)
    native = _native(values)
    assert_pathwise_equivalent(
        assembly, native, list(REGISTERS) + ["callee_call"])
