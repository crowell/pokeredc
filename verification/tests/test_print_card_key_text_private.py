...
class Setup(angr.SimProcedure):
 def run(self):
  value=self.state.memory.load(CURMAP,1);self.state.regs.h=claripy.BVV(0x66,8);self.state.regs.l=claripy.BVV(0xe3,8);self.state.regs.a=value;self.state.regs.b=value;self.jump(DONE)
def _assembly(v):
 l=symbol_location(SYMBOLS,'PrintCardKeyText');p=angr.Project(rom_window(ROM,l.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={'backend':'blob','arch':ArchPcode('z80:LE:16:default'),'base_addr':0,'entry_point':l.address});p.hook(l.address,Setup(),length=7);s=p.factory.blank_state(addr=l.address);set_assembly_registers(s,v);s.memory.store(CURMAP,v['cur_map']);m=p.factory.simulation_manager(s);m.explore(find=DONE,num_find=1);assert not m.errored;return [Endpoint(**assembly_registers(x),cur_map=x.memory.load(CURMAP,1),constraints=tuple(x.solver.constraints)) for x in m.found]
def _native(v):
 p=angr.Project(NATIVE_ELF,auto_load_libs=False);f=p.loader.find_symbol('port_print_card_key_text_private');assert f;s=p.factory.call_state(f.rebased_addr,NATIVE_STATE);store_native_registers(s,NATIVE_STATE,v);s.memory.store(NATIVE_STATE+8,v['cur_map']);m=p.factory.simulation_manager(s);m.run();assert not m.errored;return [Endpoint(**native_registers(x,NATIVE_STATE),cur_map=x.memory.load(NATIVE_STATE+8,1),constraints=tuple(x.solver.constraints)) for x in m.deadended]
@pytest.mark.skipif(not NATIVE_ELF.exists(),reason='native')
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(),reason='rom')
def test_print_card_key_text_private_pathwise_equivalence():
 v=symbolic_registers('card_key');v['cur_map']=claripy.BVS('card_key_cur_map',8);assert_pathwise_equivalent(_assembly(v),_native(v),REGISTERS)
