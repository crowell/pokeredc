...
EXPECTED=bytes.fromhex('fe40d0c3{:02x}{:02x}'.format(symbol_location(SYMS,'AISwitchIfEnoughMons').address&0xff,symbol_location(SYMS,'AISwitchIfEnoughMons').address>>8))
...
def assembly(v):
 l=symbol_location(SYMS,'JugglerAI');t=symbol_location(SYMS,'AISwitchIfEnoughMons');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_juggler_ai');t=p.loader.find_symbol('port_ai_switch_if_enough_mons');assert f and t
...
def test_juggler_ai_pathwise_equivalence():
 v=inputs('juggler_ai');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ai'))
