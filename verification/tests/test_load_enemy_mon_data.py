...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "LoadEnemyMonData")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_load_enemy_mon_data")
...
def test_load_enemy_mon_data_link_pathwise_equivalence() -> None:
    values = symbolic_registers("load_enemy_data")
    values["link_state"] = claripy.BVS("load_enemy_data_link_state", 8)
