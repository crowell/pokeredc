...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "HealEffect")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_heal_effect")
...
def test_heal_effect_symbolic_equivalence() -> None:
    inputs = symbolic_registers("hle")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_heal_effect_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "HealEffect")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("21ec79060ec3d635")
