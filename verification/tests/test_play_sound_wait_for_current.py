...
WAIT_FOR_SOUND_ADDR = 0x3748
PLAY_SOUND_ADDR = 0x23B1
...
def _assembly_endpoint() -> list[Endpoint]:
    ...
    class WaitForSoundRetSim(angr.SimProcedure):
        def run(self):
            self.jump(GB_RETURN)
    project.hook(WAIT_FOR_SOUND_ADDR, WaitForSoundRetSim())

    class PlaySoundRetSim(angr.SimProcedure):
        def run(self):
            self.jump(GB_RETURN)
    project.hook(PLAY_SOUND_ADDR, PlaySoundRetSim())
...
def test_play_sound_wait_for_current_equivalence() -> None:
    assembly = _assembly_endpoint()
    native = _native_endpoint()
    assert_pathwise_equivalent(assembly, native, ("a", "f"))


@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_play_sound_wait_for_current_exact_linked_body() -> None:
    location = symbol_location(SYMBOLS, "PlaySoundWaitForCurrent")
