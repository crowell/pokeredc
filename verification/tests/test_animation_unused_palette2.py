...
class PaletteSummary(angr.SimProcedure):
    def run(self) -> None:
        ...
        self.state.regs.b = claripy.BVV(0xFF, 8)
        self.state.regs.c = claripy.BVV(0xFF, 8)
        self.state.regs.f = claripy.BVV(0, 8)
        self.state.memory.store(R_BGP, claripy.BVV(0xFF, 8))
        self.jump(DONE)


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "AnimationUnusedPalette2")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_animation_unused_palette2_player")
...
def test_animation_unused_palette2_player_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_unused_palette2")
    values["palette"] = claripy.BVS("animation_unused_palette2_value", 8)
