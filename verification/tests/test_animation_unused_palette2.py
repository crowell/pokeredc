...
EXPECTED_ENTRY = bytes.fromhex("01ffff1812")


def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    entry = symbol_location(SYMBOLS, "AnimationUnusedPalette2")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol(
        "port_animation_unused_palette2_player"
    )
...
def test_animation_unused_palette2_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_unused_palette2")
    values["on_sgb"] = claripy.BVS("animation_unused_palette2_on_sgb", 8)
    values["palette"] = claripy.BVS("animation_unused_palette2_palette", 8)
