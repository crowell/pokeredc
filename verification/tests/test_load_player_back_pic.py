...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        old = self.state.memory.load(BATTLE_TYPE, 1)
        result = old - 1
        self.state.regs.a = result
        self.state.regs.f = (self.state.regs.f & 1) | claripy.BVV(0x02, 8)
        self.state.regs.f |= claripy.If((old & 0x0F) == 0, claripy.BVV(0x10, 8), claripy.BVV(0, 8))
        self.state.regs.f |= claripy.If(result == 0, claripy.BVV(0x40, 8), claripy.BVV(0, 8))
        self.state.regs.d = claripy.BVV(0x7E, 8)
        self.state.regs.e = claripy.If(result == 0, claripy.BVV(0x9A, 8), claripy.BVV(0x0A, 8))
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "LoadPlayerBackPic")
    ...
    project.hook(location.address, Setup(), length=0x0C)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_load_player_back_pic")
...
def test_load_player_back_pic_pathwise_equivalence() -> None:
    values = symbolic_registers("load_back_pic")
    values["battle_type"] = claripy.BVS("load_back_pic_battle_type", 8)
