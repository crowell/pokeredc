...
class OrC(angr.SimProcedure):
    """Models `or c` (a = b | c) and the Z flag it sets."""
    ...
    def run(self) -> None:  # type: ignore[override]
        self.inhibit_autoret = True
        a = self.state.regs.a | self.state.regs.c
        self.state.regs.a = a
        z = a == 0
        # shim-Z80 layout: only Z@bit6 matters (OR clears N/H/C).
        self.state.regs.f = claripy.If(z, claripy.BVV(0x40, 8), claripy.BVV(0, 8))
...
def assembly(i: dict) -> list[E]:
    loc = symbol_location(SYMBOLS, "ZeroSpriteBuffer")
    q = symbol_location(SYMBOLS, "ZeroSpriteBuffer.nextByteLoop").address
    p = angr.Project(
    ...
    p.hook(q, XorA(q + 1), length=1)         # xor a
...
def native(i: dict) -> list[E]:
    ...
    fn = p.loader.find_symbol("port_zero_sprite_buffer_step")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "ZeroSpriteBuffer")
    assert linked_bytes(ROM, loc, 11) == bytes.fromhex("018801af220b78b120f9c9")
