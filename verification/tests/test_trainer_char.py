...
def _assembly(values: dict[str, claripy.ast.BV]):
    location = symbol_location(SYMBOLS, "TrainerChar")
    target = symbol_location(SYMBOLS, "PlaceCommandCharacter")
    expected = bytes.fromhex("d511581a1840")
...
def _native(values: dict[str, claripy.ast.BV]):
    ...
    function = project.loader.find_symbol("port_trainer_char")
...
def test_trainer_char_pathwise_equivalence() -> None:
    values = symbolic_registers("trainer_char")
