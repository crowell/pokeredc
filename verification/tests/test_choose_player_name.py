"""Proof for the custom-name prefix of ChoosePlayerName."""
...
from verification.harness.sm83_shims import Sm83StoreAImmediate, Sm83XorA
...
EXPECTED = bytes.fromhex("cd126a11a86acd6c6afa26cca7280e21f26acdd66a1158d1cdec69181f2158d1afea7dd0cd9665")
...
def _endpoint(state: angr.SimState, base: int) -> Endpoint:
    ...
    address = base + W_CURRENT_MENU_ITEM
    naming = base + W_NAMING_SCREEN_TYPE
    return Endpoint(
        a=registers["a"],
        f=registers["f"],
        h=registers["h"],
        l=registers["l"],
        menu=state.memory.load(address, 1),
        naming_type=state.memory.load(naming, 1),
        constraints=tuple(state.solver.constraints),
    )


def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "ChoosePlayerName")
    base = loc.address
    ...
    project.hook(base + 0x1D, LoadHLImmediate(W_PLAYER_NAME, base + 0x20), length=3)
    project.hook(base + 0x20, Sm83XorA(base + 0x21), length=1)
    project.hook(base + 0x21, Sm83StoreAImmediate(W_NAMING_SCREEN_TYPE, base + 0x24), length=3)
    project.hook(base + 0x24, CallBoundary(DONE), length=3)
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_choose_player_name")
...
def test_choose_player_name_custom_prefix_pathwise_equivalence() -> None:
    inputs = symbolic_registers("choose_player_name")
    assert_pathwise_equivalent(
        _assembly(inputs),
        _native(inputs),
        ("a", "f", "h", "l", "menu", "naming_type"),
    )


def test_choose_player_name_exact_linked_prefix() -> None:
    loc = symbol_location(SYMBOLS, "ChoosePlayerName")
    assert linked_bytes(ROM, loc, 0x27) == EXPECTED
