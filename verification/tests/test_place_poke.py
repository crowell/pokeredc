...
def _assembly(values: dict[str, claripy.ast.BV]):
    location = symbol_location(SYMBOLS, "PlacePOKe")
    target = symbol_location(SYMBOLS, "PlaceCommandCharacter")
    expected = bytes.fromhex("d5116a1a1828")
...
def _native(values: dict[str, claripy.ast.BV]):
    ...
    function = project.loader.find_symbol("port_place_poke")
...
def test_place_poke_pathwise_equivalence() -> None:
    values = symbolic_registers("place_poke")
