...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "LeaveMapAnim")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_leave_map_anim")
...
def test_leave_map_anim_symbolic_equivalence() -> None:
    inputs = symbolic_registers("lma")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_leave_map_anim_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "LeaveMapAnim")
