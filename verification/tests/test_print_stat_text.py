...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        self.state.regs.h = claripy.BVV(0x76, 8)
        self.state.regs.l = claripy.BVV(0x9F, 8)
        self.state.regs.c = claripy.BVV(0x40, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "PrintStatText")
...
def test_print_stat_text_setup_pathwise_equivalence() -> None:
    values = symbolic_registers("print_stat_text")
