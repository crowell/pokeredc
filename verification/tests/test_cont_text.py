...
def _memory(state: angr.SimState, base: int) -> claripy.ast.BV:
    return claripy.Concat(
        ...
        *(state.memory.load(base + REGION + i, 1) for i in range(REGION_LENGTH)),
    )
...
def _assembly(values: dict[str, claripy.ast.BV], link_state: claripy.ast.BV,
    ...
    location = symbol_location(SYMBOLS, "_ContText")
    assert linked_bytes(ROM, location, 32) == bytes.fromhex(
        "3eeeeaf2c4cd3a1bd5cd9838d13e7feaf2c4d5cd181bcd181b21e1c4d1c3e819"
    )
    ...
    q = location.address
    project.hook(q, LoadArrow(), length=2)
    project.hook(q + 2, StoreArrow(), length=3)
    project.hook(q + 5, Delay3Boundary(), length=3)
    project.hook(q + 8, PushDE(), length=1)
    project.hook(q + 9, ManualBoundary(), length=3)
    project.hook(q + 12, PopDE(), length=1)
    project.hook(q + 13, LoadSpace(), length=2)
    project.hook(q + 15, StoreArrow(), length=3)
    project.hook(q + 18, PushDE(), length=1)
    project.hook(q + 19, ScrollSite(), length=3)
    project.hook(q + 22, ScrollSite(), length=3)
    project.hook(q + 25, LoadHL(), length=3)
    project.hook(q + 28, PopDE(), length=1)
...
def _native(values: dict[str, claripy.ast.BV], link_state: claripy.ast.BV,
    ...
    function = project.loader.find_symbol("port_cont_text")
...
def test_cont_text_pathwise_equivalence(link_value: int) -> None:
    values = symbolic_registers("cont_text")
    values["arrow"] = claripy.BVS("cont_text_arrow", 8)
    for offset in range(REGION_LENGTH):
        values[f"region{offset}"] = claripy.BVS(f"cont_text_region_{offset}", 8)
