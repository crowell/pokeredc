...
def assembly(v):
 l=symbol_location(SYMS,'KogaAI');t=symbol_location(SYMS,'AIUseXAttack');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_koga_ai');t=p.loader.find_symbol('port_ai_use_x_attack');assert f and t
...
def test_koga_ai_pathwise_equivalence():
 v=inputs('koga_ai');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ai'))
