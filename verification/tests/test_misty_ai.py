...
EXPECTED=bytes.fromhex('fe40d0c3{:02x}{:02x}'.format(symbol_location(SYMS,'AIUseXDefend').address&0xff,symbol_location(SYMS,'AIUseXDefend').address>>8))
...
def assembly(v):
 l=symbol_location(SYMS,'MistyAI');t=symbol_location(SYMS,'AIUseXDefend');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_misty_ai');t=p.loader.find_symbol('port_ai_use_x_defend');assert f and t
...
def test_misty_ai_pathwise_equivalence():
 v=inputs('misty_ai');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ai'))
