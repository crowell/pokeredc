...
EXPECTED=bytes.fromhex('2163d0f0f3a728032168d0cb562010cbd621a87b060fcdd63521b27fc3493c0e32cd393721537b060fc3d635')
...
def assembly(v):
 l=symbol_location(SYMS,'FocusEnergyEffect_');pt=symbol_location(SYMS,'PrintText')
 ...
 p.hook(b+11,Sm83BitAtHl(2,b+13),length=2)                  # bit GETTING_PUMPED,[hl]
 p.hook(b+15,Sm83SetAtHl(2,b+17),length=2)                  # set GETTING_PUMPED,[hl]
 ...
 p.hook(b+33,ACall(2,b+36),length=3)                          # call DelayFrames
 p.hook(b+41,ACall(3,None),length=3)                        # jp Bankswitch tail (jpfar PrintButItFailedText_)
...
def native(v):
 ...
 f=p.loader.find_symbol('port_focus_energy_effect');assert f
...
def test_focus_energy_effect_pathwise_equivalence():
 v=inputs('focus_energy_effect');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'ps2','es2','turn','ib0','ib1','ib2','ib3'))
