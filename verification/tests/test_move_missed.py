...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "MoveMissed")
    ...
    state.regs.a = values["move_effect"]
...
def test_move_missed_pathwise_equivalence() -> None:
    values = symbolic_registers("move_missed")
    values["move_effect"] = claripy.BVS("move_missed_effect", 8)
