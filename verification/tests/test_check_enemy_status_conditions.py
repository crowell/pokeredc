...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        status = self.state.memory.load(STATUS, 1)
        self.state.regs.h = claripy.BVV(0xCF, 8)
        self.state.regs.l = claripy.BVV(0xE9, 8)
        self.state.regs.a = status & 7
        self.state.regs.f = claripy.BVV(0x10, 8) | claripy.If(self.state.regs.a == 0, claripy.BVV(0x40, 8), claripy.BVV(0, 8))
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "CheckEnemyStatusConditions")
    ...
    state.memory.store(STATUS, values["enemy_mon_status"])
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_check_enemy_status_conditions")
    assert function is not None
    state = project.factory.call_state(function.rebased_addr, NATIVE_STATE)
    store_native_registers(state, NATIVE_STATE, values)
    state.memory.store(NATIVE_STATE + 8, values["enemy_mon_status"])
    manager = project.factory.simulation_manager(state)
    manager.run()
    assert not manager.errored
...
def test_check_enemy_status_conditions_pathwise_equivalence() -> None:
    values = symbolic_registers("enemy_status")
    values["enemy_mon_status"] = claripy.BVS("enemy_status_value", 8)
