...
class Boundary(angr.SimProcedure):
    """The tail `jp $36d6` (bankswitch routine) is the path boundary."""
...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "CallBankF")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_call_bank_f")
...
def test_call_bank_f_symbolic_equivalence() -> None:
    inputs = symbolic_registers("cbf")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_call_bank_f_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "CallBankF")
    assert linked_bytes(ROM, loc, 5) == bytes.fromhex("060fc3d635")
