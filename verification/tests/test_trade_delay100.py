...
class DelayFrames100(angr.SimProcedure):
...
def _assembly(values: dict[str, claripy.ast.BV]) -> Endpoint:
    location = symbol_location(SYMBOLS, "Trade_Delay100")
    ...
    project.hook(base + 2, DelayFrames100(), length=3)
...
def _native(values: dict[str, claripy.ast.BV]) -> Endpoint:
    ...
    function = project.loader.find_symbol("port_trade_delay100")
...
def test_trade_delay100_pathwise_equivalence() -> None:
    values = symbolic_registers("trade_delay100")
    values["frames_waited"] = claripy.BVS("trade_delay100_frames", 8)
