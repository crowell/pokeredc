...
class SetupHL(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.l = claripy.BVV(0x15, 8)
        self.jump(self.state.addr + 3)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "MainInBattleLoop")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_main_in_battle_loop")
...
def test_main_in_battle_loop_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("main_in_battle_loop")
    values["hp_low"] = claripy.BVS("main_in_battle_loop_hp_low", 8)
    values["hp_high"] = claripy.BVS("main_in_battle_loop_hp_high", 8)
