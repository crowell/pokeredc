...
RETURN_H = 0x35
RETURN_L = 0xE4
EXPECTED = bytes.fromhex("0605212c7e18f3")
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "TextScript_PokemonCenterPC")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_text_script_pokemon_center_pc")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(),
                    reason="run `make red`")
def test_text_script_pokemon_center_pc_pathwise_equivalence() -> None:
    values = symbolic_registers("pokemon_center_pc")
    values["loaded"] = claripy.BVS("pokemon_center_pc_loaded", 8)
    values["romb"] = claripy.BVS("pokemon_center_pc_romb", 8)
    for name in REGISTERS:
        values[f"callback_{name}"] = (
            claripy.Concat(claripy.BVS("pokemon_center_pc_callback_flags", 4),
                           claripy.BVV(0, 4))
            if name == "f" else
            claripy.BVS(f"pokemon_center_pc_callback_{name}", 8))
    values["callback_loaded"] = claripy.BVS("pokemon_center_pc_callback_loaded", 8)
    values["callback_mapper"] = claripy.BVS("pokemon_center_pc_callback_mapper", 8)
    for name in REGISTERS:
        values[f"out_{name}"] = (
            claripy.Concat(claripy.BVS("pokemon_center_pc_out_flags", 4),
                           claripy.BVV(0, 4))
            if name == "f" else
            claripy.BVS(f"pokemon_center_pc_out_{name}", 8))
