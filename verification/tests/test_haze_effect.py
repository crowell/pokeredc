...
def _assembly(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    loc = symbol_location(SYMBOLS, "HazeEffect")
    base = loc.address
...
def _native(inputs: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_haze_effect")
...
def test_haze_effect_symbolic_equivalence() -> None:
    inputs = symbolic_registers("he")
...
@pytest.mark.skipif(not ROM.exists() or not SYMBOLS.exists(), reason="run `make red`")
def test_haze_effect_exact_linked_body() -> None:
    loc = symbol_location(SYMBOLS, "HazeEffect")
    assert linked_bytes(ROM, loc, 8) == bytes.fromhex("21da790604c3d635")
