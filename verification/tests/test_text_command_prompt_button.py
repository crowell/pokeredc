...
class E:
 ...
 arrow:claripy.ast.BV;link:claripy.ast.BV;constraints:tuple[claripy.ast.Bool,...]
def inputs(p):
 ...
 v['arrow']=claripy.BVS(f'{p}_arrow',8);v['link']=claripy.BVV(0,8)
 return v
def setup(s,v,native:bool):
 o=NM if native else 0
 s.memory.store(o+WLINKSTATE,v['link']);s.memory.store(o+ARROW,v['arrow'])
...
def assembly(v):
 ...
 p.hook(b+0x0e,ManualTextScrollSite(b+0x11),length=3)           # call ManualTextScroll (proved boundary)
 p.hook(b+0x11,PopBC(b+0x12),length=1)                          # pop bc
 p.hook(b+0x12,LoadAConst(0x7f,b+0x14),length=2)                # ld a,' '
 ...
 for x in m.found:
  ...
  out.append(E(**{**{k:v for k,v in ar.items() if k not in ('h','l')},'hl':claripy.Concat(x.regs.l,x.regs.h)},arrow=x.memory.load(ARROW,1),link=x.memory.load(WLINKSTATE,1),constraints=tuple(x.solver.constraints)))
 return out
def native(v):
 p=angr.Project(ELF,auto_load_libs=False)
 f=p.loader.find_symbol('port_text_command_prompt_button');assert f
