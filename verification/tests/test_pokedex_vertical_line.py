...
def begin(i:dict[str,claripy.ast.BV])->E:
 loc=symbol_location(SYMBOLS,"DrawPokedexVerticalLine");p=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":loc.address});p.hook(loc.address+7,Bound(LOOP),length=1);s=p.factory.blank_state(addr=loc.address);set_assembly_registers(s,i);m=p.factory.simulation_manager(s);m.explore(find=LOOP);x=m.found[0];return E(**assembly_registers(x),written=i["written"],cont=claripy.BVV(1,8),constraints=tuple(x.solver.constraints))
def step(i:dict[str,claripy.ast.BV])->list[E]:
 loc=symbol_location(SYMBOLS,"DrawPokedexVerticalLine");loop=loc.address+7;p=angr.Project(rom_window(ROM,loc.bank),auto_load_libs=False,rebase_granularity=0x100,main_opts={"backend":"blob","arch":ArchPcode("z80:LE:16:default"),"base_addr":0,"entry_point":loop});p.hook(loop,Store(loop+1),length=1);p.hook(loop+1,Sm83AddHlRegisterPair("de",loop+2),length=1);p.hook(loop+2,Sm83XorImmediate(1,loop+4),length=2);p.hook(loop+4,Sm83DecRegister("c",loop+5),length=1);p.hook(loop+7,Bound(RETURN),length=1);s=p.factory.blank_state(addr=loop);set_assembly_registers(s,i);s.globals["written"]=i["written"];m=p.factory.simulation_manager(s);m.stashes["found"]=[]
 while m.active:
...
def test_begin()->None:
 i=symbolic_registers("pokedex_vertical_begin");i["written"]=claripy.BVS("pokedex_vertical_begin_written",8);assert_pathwise_equivalent([begin(i)],native("port_draw_pokedex_vertical_line_begin",i,False),(*REGISTERS,"cont"))
@pytest.mark.skipif(not NATIVE_ELF.exists(),reason="native")
def test_step()->None:
 i=symbolic_registers("pokedex_vertical_step");i["written"]=claripy.BVS("pokedex_vertical_step_written",8);assert_pathwise_equivalent(step(i),native("port_draw_pokedex_vertical_line_step",i,True),(*REGISTERS,"written","cont"))
def test_body()->None:
 loc=symbol_location(SYMBOLS,"DrawPokedexVerticalLine");assert linked_bytes(ROM,loc,15)==bytes.fromhex("0e091114003e717719ee010d20f9c9")
