...
def _inputs(prefix: str) -> dict[str, claripy.ast.BV]:
    ...
    values["layout"] = claripy.BVS(f"{prefix}_layout", 8)
...
def _memory_endpoint(state: angr.SimState, base: int) -> claripy.ast.BV:
    return claripy.Concat(
        state.memory.load(base + H_LAYOUT, 1),
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    state.memory.store(NATIVE_MEMORY + H_LAYOUT, values["layout"])
    state.memory.store(NATIVE_MEMORY + DESTINATION, values["destination_byte"])
    state.memory.store(NATIVE_MEMORY + SOURCE, claripy.BVV(TX_NEXT, 8))
