...
class Setup(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.d = claripy.BVV(0xD1, 8)
        self.state.regs.e = claripy.BVV(0x63, 8)
        self.jump(DONE)

def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "SetupOwnPartyPokeballs")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_setup_own_party_pokeballs")
...
def test_setup_own_party_pokeballs_pathwise_equivalence() -> None:
    values = symbolic_registers("setup_own_pokeballs")
