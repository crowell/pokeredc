...
EXPECTED=linked_bytes(ROM,symbol_location(SYMS,'FuchsiaBillsGrandpasHouse_Script'),4)
...
def assembly(v):
 l=symbol_location(SYMS,'FuchsiaBillsGrandpasHouse_Script');assert linked_bytes(ROM,l,len(EXPECTED))==EXPECTED
...
def native(v):
 p=angr.Project(ELF,auto_load_libs=False);f=p.loader.find_symbol('port_fuchsia_bills_grandpas_house_script');t=p.loader.find_symbol('port_enable_auto_text_box_drawing');assert f and t
...
def test_fuchsia_bills_grandpas_house_script_pathwise_equivalence():
 v=inputs('fuchsia_bills_grandpas_house_script');assert_pathwise_equivalent(assembly(v),native(v),(*REGISTERS,'control','wait','at'))
