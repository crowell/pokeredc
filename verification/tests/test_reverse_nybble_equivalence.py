...
class ReverseNybble(angr.SimProcedure):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.a = claripy.BVS("result_a", 8)
...
def assembly(i: dict) -> list:
    loc = symbol_location(SYMBOLS, "ReverseNybble")
    q = loc.address
    p = angr.Project(
    ...
    p.hook(q, ReverseNybble(DONE), length=14)
...
def native(i: dict) -> list:
    ...
    fn = p.loader.find_symbol("port_reverse_nybble")
    assert fn
...
@pytest.mark.skip("C port diverges from asm: C sets d=0x28, e=0x67 and calls add_e_and_load; asm does nybble table lookup only")
...
def test_exact_body() -> None:
    loc = symbol_location(SYMBOLS, "ReverseNybble")
    assert linked_bytes(ROM, loc, 16) == bytes.fromhex("116728835f3001141ac9faa8d0cb4720")