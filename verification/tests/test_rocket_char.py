...
def _assembly(values: dict[str, claripy.ast.BV]):
    location = symbol_location(SYMBOLS, "RocketChar")
    target = symbol_location(SYMBOLS, "PlaceCommandCharacter")
    expected = bytes.fromhex("d511631a182e")
...
def _native(values: dict[str, claripy.ast.BV]):
    ...
    function = project.loader.find_symbol("port_rocket_char")
...
def test_rocket_char_pathwise_equivalence() -> None:
    values = symbolic_registers("rocket_char")
