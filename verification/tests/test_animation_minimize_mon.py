...
class SetupPrefix(angr.SimProcedure):
    def run(self) -> None:
        self.inhibit_autoret = True
        state = self.state.copy()
        state.regs.a = claripy.BVV(0, 8); state.regs.f = claripy.BVV(0x40, 8)
        state.regs.b = claripy.BVV(3, 8); state.regs.c = claripy.BVV(0x10, 8)
        state.regs.h = claripy.BVV(0xc6, 8); state.regs.l = claripy.BVV(0xe8, 8)
        self.successors.add_successor(state, self.addr + 8, claripy.BoolV(True), "Ijk_Boring")
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "AnimationMinimizeMon")
    fill = symbol_location(SYMBOLS, "FillMemory")
    ...
    project.hook(location.address, SetupPrefix(), length=8)
    project.hook(fill.address, Boundary(), length=1)
    state = project.factory.blank_state(addr=location.address); set_assembly_registers(state, values)
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    project = angr.Project(NATIVE_ELF, auto_load_libs=False); function = project.loader.find_symbol("port_animation_minimize_mon"); assert function is not None
...
def test_animation_minimize_mon_entry_pathwise_equivalence() -> None:
    values = symbolic_registers("animation_minimize_mon")
