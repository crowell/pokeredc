...
class SetupHLBC(angr.SimProcedure):
    def run(self) -> None:  # type: ignore[override]
        ...
        self.state.regs.c = claripy.BVV(9, 8)
...
def _assembly(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    location = symbol_location(SYMBOLS, "PrintMenuItem")
...
def _native(values: dict[str, claripy.ast.BV]) -> list[Endpoint]:
    ...
    function = project.loader.find_symbol("port_print_menu_item")
...
def test_print_menu_item_setup_pathwise_equivalence() -> None:
    values = symbolic_registers("print_menu_item")
    values["auto_bg"] = claripy.BVS("print_menu_item_auto_bg", 8)
