...
static port_u16
double_pair(port_u8 high, port_u8 low)
{
	return (port_u16)(((port_u16)high << 8) | low);
}

__attribute__((noinline, used)) port_u8
port_vblank_copy_double_setup(struct vblank_copy_double_state *state)
{
...
__attribute__((noinline, used)) port_u8
port_vblank_copy_double_step(struct vblank_copy_double_state *state)
{
	port_u16 sp = double_pair(state->sp_high, state->sp_low);
	port_u16 hl;
	...
	for (pair = 0; pair < 4; pair++) {
		...
		for (repeat = 0; repeat < 2; repeat++) {
			state->written[output] = state->registers.e;
			state->write_h[output] = high;
			state->write_l[output++] = low;
			low++;
		}
		for (repeat = 0; repeat < 2; repeat++) {
			state->written[output] = state->registers.d;
			state->write_h[output] = high;
			state->write_l[output++] = low;
			if (pair != 3 || repeat != 1)
				low++;
	...
	hl = (port_u16)(double_pair(high, low) + 1);
...
/* Port of VBlankCopyDouble in home/vcopy.asm. */
__attribute__((noinline, used)) void
port_vblank_copy_double(struct vblank_copy_double_state *state,
	port_u8 *memory)
{
	port_u8 continuation = port_vblank_copy_double_setup(state);
	port_u16 source;
	port_u16 address;
	port_u8 i;

	while (continuation != 0) {
		source = double_pair(state->sp_high, state->sp_low);
		for (i = 0; i < 8; i++)
			state->source_bytes[i] = memory[(port_u16)(source + i)];
		continuation = port_vblank_copy_double_step(state);
		for (i = 0; i < 16; i++) {
			address = double_pair(state->write_h[i], state->write_l[i]);
