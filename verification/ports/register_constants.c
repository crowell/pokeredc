...
/* Complete port of GBFadeInFromWhite in home/fade.asm. */
__attribute__((noinline, used)) void
port_gb_fade_in_from_white(struct cpu_register_state *state, port_u8 *memory)
{
	set_hl(state, 0x2121);
	state->b = 3;
	do {
	...
	const port_u8 *memory)
{
	...
	state->a = memory[0xff8b];
	state->b = state->a;
	state->a = memory[0xff8c];
	state->a = (port_u8)((state->a << 4) | (state->a >> 4));
	left = state->a;
	right = state->b;
	wide = (port_u16)left + right;
	state->a = (port_u8)wide;
	state->f = 0;
	if (state->a == 0)
		state->f |= PORT_FLAG_Z;
...
/* Complete port of GetPointerWithinSpriteStateData1. */
__attribute__((noinline, used)) void
