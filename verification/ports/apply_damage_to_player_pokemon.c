...
struct apply_damage_player_state {
	struct cpu_register_state registers;
	port_u8 damage_high;
	port_u8 damage_low;
};

/* Port of ApplyDamageToPlayerPokemon through the zero-damage branch. */
__attribute__((noinline, used)) void
port_apply_damage_to_player_pokemon(struct apply_damage_player_state *state)
{
