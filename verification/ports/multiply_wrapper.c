#include "port_state.h"

void port_multiply(struct multiply_state *state);

/* Port of the home-bank Multiply wrapper in home/math.asm. */
__attribute__((noinline, used)) void
port_multiply_wrapper(struct multiply_wrapper_state *state)
{
	struct cpu_register_state *registers = &state->multiply.registers;
	port_u8 saved_b = registers->b;
	port_u8 saved_c = registers->c;
	port_u8 saved_h = registers->h;
	port_u8 saved_l = registers->l;
	port_u8 saved_bank = state->loaded_rom_bank;

	registers->h = 0x7d;
	registers->l = 0x41;
	registers->b = 0x0d;
	registers->a = registers->b;
	state->loaded_rom_bank = registers->a;
	state->mapper_bank = registers->a;
	registers->b = 0x35;
	registers->c = 0xe4;
	port_multiply(&state->multiply);
	registers->a = saved_bank;
	state->loaded_rom_bank = saved_bank;
	state->mapper_bank = saved_bank;
	registers->b = saved_b;
	registers->c = saved_c;
	registers->h = saved_h;
	registers->l = saved_l;
}
