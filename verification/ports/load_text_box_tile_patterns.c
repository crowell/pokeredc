...
void port_far_copy_data2(struct far_copy_data2_state *state,
	port_u8 *memory);
void port_load_text_box_tile_patterns_on(struct cpu_register_state *state,
	port_u8 *memory);

/* Port of LoadTextBoxTilePatterns in home/load_font.asm. */
	...
	struct load_text_box_tile_patterns_state *state, port_u8 *memory)
{
	...
	if ((registers->a & 0x80) != 0) {
		port_load_text_box_tile_patterns_on(registers, memory);
		return;
	}
	registers->h = 0x62;
	registers->l = 0x88;
