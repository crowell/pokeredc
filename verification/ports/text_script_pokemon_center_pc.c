...
#define ACTIVATE_PC_BANK 5u
#define ACTIVATE_PC 0x7e2cu
...
/* Port of TextScript_PokemonCenterPC in home/map_objects.asm. */
__attribute__((noinline, used)) void
port_text_script_pokemon_center_pc(
	struct text_script_pokemon_center_pc_state *state, port_u8 *memory)
{
	struct bankswitch_state bankswitch = {0};
	struct hold_text_display_open_state hold = {0};

	state->registers.b = ACTIVATE_PC_BANK;
	state->registers.h = (port_u8)(ACTIVATE_PC >> 8);
	state->registers.l = (port_u8)ACTIVATE_PC;

	/* Bankswitch's indirect JP enters ActivatePC with this setup. */
	state->callback_call[0] = ACTIVATE_PC_BANK;
	...
	state->callback_call[6] = (port_u8)(ACTIVATE_PC >> 8);
	state->callback_call[7] = (port_u8)ACTIVATE_PC;
	state->callback_call[8] = ACTIVATE_PC_BANK;
	state->callback_call[9] = ACTIVATE_PC_BANK;
