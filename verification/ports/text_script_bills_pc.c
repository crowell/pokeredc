...
#define BILLS_PC_BANK 8u
#define BILLS_PC 0x54c2u
...
/* Port of TextScript_BillsPC in home/map_objects.asm. */
__attribute__((noinline, used)) void
port_text_script_bills_pc(struct text_script_bills_pc_state *state,
	port_u8 *memory)
{
	...
	state->registers.b = BILLS_PC_BANK;
	state->registers.h = (port_u8)(BILLS_PC >> 8);
	state->registers.l = (port_u8)BILLS_PC;

	/* Bankswitch's indirect JP enters BillsPC_ with this setup. */
	state->callback_call[0] = BILLS_PC_BANK;
	...
	state->callback_call[6] = (port_u8)(BILLS_PC >> 8);
	state->callback_call[7] = (port_u8)BILLS_PC;
	state->callback_call[8] = BILLS_PC_BANK;
	state->callback_call[9] = BILLS_PC_BANK;
