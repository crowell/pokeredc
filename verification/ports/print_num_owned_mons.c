...
#define POKEDEX_OWNED_BYTES 19u
...
/* Port of PrintNumOwnedMons in engine/menus/main_menu.asm. */
__attribute__((noinline, used)) void
port_print_num_owned_mons(struct print_number_state *state, port_u8 *memory)
{
	...
	count.registers.b = POKEDEX_OWNED_BYTES;
	count.num_set_bits = memory[W_NUM_SET_BITS];
	count.fetched = memory[W_POKEDEX_OWNED];
